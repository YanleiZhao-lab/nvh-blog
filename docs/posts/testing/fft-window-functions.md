...
同样的信号、同样的采样率、同样的平均次数，只把分析设置里的窗从汉宁换成平顶，50 Hz 峰的读数就变了 1 dB 还多——标定台上正弦幅值到底该信哪一次？更困惑的在另一头：两个频率只差 4 Hz 的音，汉宁窗下清清楚楚是两个峰，换成分辨率更高的平顶窗反而只剩一个——「更精确的窗」怎么把峰测丢了？这两个反常识现象都源于同一件事：FFT 假设你截的那段数据在不断重复，而窗函数决定拼接处接得多平。接得平，能量各归各；接不平，能量洒满整条谱线。窗函数的全部学问，就是在这条拼接缝上做取舍——而取舍一旦理解，换窗读数变就从玄学变成了可预算的账。

## 一、为什么需要窗函数

生活里有个贴切的比方：搬家装车。家具（信号的能量）要么散装硬塞（矩形窗——两端说断就断，缝里崩出的碎屑洒一路），要么每件先裹上厚薄不一的气泡膜（汉宁、平顶等渐变窗——两端缓慢收拢，路上不磕碰，但占地方）。气泡膜裹得越厚（旁瓣越低、幅值越准），单件家具占的车厢就越长（主瓣越宽、分辨力越差）——平顶窗是每件裹三层的厚包装，单件幅值量得分毫不差，可两件小家具挨得太近就被包成一团、分不开了。整篇文章记住这辆货车即可：每换一种窗，就是在裹多厚与装多少件之间重新选一次。


再看一组具体数字，感受这条拼接缝的能量级。1 秒采样时长、1 Hz 分辨率下，一个幅值 1.0 的 50.5 Hz 正弦（恰好落在 50 与 51 两条谱线正中间的最坏位置）：不加窗直接做 FFT，最大幅值误差 3.9 dB——读数可能只有 0.64；加汉宁窗，误差缩到 1.4 dB；加平顶窗，误差小于 0.01 dB，读数回到 0.999。代价同样具体：汉宁窗下这条谱线的能量摊到两侧各约 1.5 条谱线，平顶窗摊到两侧各约 3.43 条——也就是说，若 3 Hz 之外还有另一个等幅的音，平顶窗的两滩能量就会叠在一起。数字摆在桌面上，「换窗读数变」便不再是玄学：每一次换窗都是在重新分配这条缝上的账。


本文把这笔账摊开算清楚：先讲泄漏从哪来（第一节），再给出比较窗函数的三个频域指标（第二节），逐个解读六种常用窗的脾气（第三节），补上加窗后必须乘回来的校正因子（第四节），最后用一段可运行的 Python 代码把三种窗的频谱画在一起（第五、六节），并给出一张按场景查表的决策清单（第七节）。
...
*图 1　整周期截断无泄漏，非周期截断产生全带宽泄漏（图源：网络官方公开资料）*

**导读**：下面这个公式回答的问题是「加窗这个动作在数学上到底是什么」——其中 $w(n)$ 对应物理里的「气泡膜」：一段先升后降的包络，逐点与信号相乘，把采样窗两端压到零。
...
**导读**：这个卷积关系回答的问题是「时域裹膜到频域变成了什么」——其中 $W(f)$ 对应物理里的「膜的频域指纹」：信号的每条谱线都被这枚指纹盖章，指纹的形状（主瓣多宽、旁瓣多低）直接印到结果上。
...
**导读**：这个公式回答的问题是「一种窗到底占用多少条谱线的车位」——其中分子上的 $\sum w^2(n)$ 对应物理里的「膜的能量含量」，分母上的 $(\sum w(n))^2$ 对应「膜的净增益」，两者之比乘以 $N$ 就是该窗等效占用的谱线宽度。
...
下表给出常用窗函数的主要特性（数据取自相关测试技术手册表 1.1）：
...
这张表就是「裹膜价目表」，每一列对应一个取舍维度：旁瓣越低（膜的缓冲越好），ENBW 越宽（占的车位越长），最大幅值误差越小（单件量得越准）。没有任何一行在四列里同时最优——这正是取舍二字的含义，也是下一节逐窗解读的线索。
...
回到搬家的比方：矩形窗就是不裹膜硬塞——家具本身方方正正、能贴合车厢（整周期或瞬态信号）时，这是最省空间的方式；可要是形状参差（随机信号），硬塞的后果就是缝里崩出的碎屑洒满整节车厢（-13 dB 旁瓣、全带宽泄漏）。

### 3.2 汉宁窗（Hanning）

汉宁窗是随机信号通用分析中最常用的窗，其时域表达式为：

**导读**：这个表达式回答的问题是「最常用的那层标准气泡膜长什么样」——起止为零、中心为 1 的余弦渐变，正是「两端缓慢收拢」这句物理描述的数学形状。
...
*图 2　汉宁窗（中）作用于随机信号（左上），使时域端点平滑归零（右下），抑制泄漏（图源：网络官方公开资料）*

![汉宁窗时域形状（左）及其对周期/非周期正弦波的频域影响（右）](/images/fft-window-functions/fig5-hann-shape.png)
*图 3　汉宁窗的时域形状与频域效果：对周期信号反而展宽谱线，对非周期信号显著抑制泄漏（图源：网络官方公开资料）*
...
*图 4　汉宁窗与平顶窗的频谱对比：平顶窗幅值精度高，但谱线更宽（图源：网络官方公开资料）*

![相距 4 Hz 的两个周期截断音：汉宁窗（绿）分出两个峰，平顶窗（蓝）只显示一个峰](/images/fft-window-functions/fig11-hann-vs-flattop-two-tones.png)
*图 5　两音间隔 4 Hz、分辨率 1 Hz 时：汉宁窗能分开两个峰，平顶窗因泄漏叠加只显示一个峰（图源：网络官方公开资料）*

开篇那个「平顶窗把两个峰测成一个」的现象，答案就在图 5：平顶窗这层三层厚包装的主瓣宽到 3.43 条谱线，两个峰各自的主瓣在中间叠成一片——幅值各自量得很准，「有几个峰」这个信息却丢了。而标定台上单频幅值差 1 dB 的疑惑同样由它给出答案：汉宁窗的最大幅值误差 1.4 dB 发生在频率落在两条谱线正中间时，平顶窗小于 0.01 dB——单音测幅值，平顶窗就是为此而生。
...
::: info 幅值校正的实现
上例中的 `scale = N / np.sum(win)` 即幅值校正因子 $K_A$：汉宁窗给出 2.0，scipy 的 flattop 给出约 4.2（与相关技术手册表 1.2 的 4.18 为不同系数的平顶窗实现，量级一致）。
:::
...
- <strong>相关测试技术手册</strong> — 第 1.3 节"泄漏与加窗"，表 1.1 时窗特性、表 1.2 窗校正因子
- **公开技术资料** — "Window Types"、"Window Correction Factors"、"Leakage and Windows"
