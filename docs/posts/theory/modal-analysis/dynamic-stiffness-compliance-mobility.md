...
注意动刚度不是常数，它就是运动方程括号里的那一项。共振点处其幅值跌到 $\omega_n c = 2\zeta k$，阻尼越小谷越深；低频趋于 $k$，高频趋于 $-\omega^2 m$（幅值以 40 dB/dec 上升）。多自由度系统里对应**动刚度矩阵** $Q = K - \omega^2 M + j\omega C$（LMS 理论手册的模态灵敏度分析用的正是这个量）。
