...
一场车身模态试验做到第三天，进度卡在了一张相干函数图上：0～40 Hz 低频段塌到 0.7 以下，共振峰附近还挂着毛刺。现场换了更大推力的激振器、重新粘贴了传感器、加到 64 次平均，曲线依旧。有人提议把信号换成伪随机：帧内周期化、矩形窗零泄漏，几十秒后低频段相干抬回 0.95 以上。同一个人、同一台设备、同一个结构，只因换了一种激励波形，数据质量判若两批——这背后的差别不在硬件，而在于激励信号的能量是否恰好落在傅里叶变换"看得见"的频率网格上。反过来说：如果一种信号天生就按网格供能、还把峰值压得足够低，那它为什么没有被用在所有模态试验里？本文回答这个问题。
...
换个角度看这件事。把采集窗内的傅里叶变换想象成一间只在整点发车的环线巴士：车按固定间隔 Δf=1/T 一班一班走，乘客（信号能量）只有恰好踩着班次到站，才能整整齐齐坐上车、一根谱线一个位置；来早了或来晚了（频率不落在网格上），就只能被拆散塞进前后两班车的角落里——这就是泄漏，能量从一根谱线摊到相邻多根谱线上，幅值再也读不准。纯随机信号是随时涌到站台上的人流，几乎没人踩得准班次，所以必须用汉宁窗这种"疏导员"强行把队伍掐头去尾压回整周期；伪随机则相当于按班车时刻表逐人预约发牌——每根谱线的频率天生就是班次间隔的整数倍，人人有座、对号入座，矩形窗原样放行即可。整个伪随机技术体系，就是围绕"怎样让能量踩准班次、又怎样在车容量（功放电压上限）固定的前提下多运人"这两件事展开。
...
- <strong>波峰因子（Crest Factor，亦称峰值因子）</strong>：峰值与均方根值（RMS）之比。正弦波为 1.41，随机信号约 3～4。波峰因子越高，功放与激振器在同样电压上限下的有效激励能量越低
...
这个公式回答的问题是："怎样用有限的正弦波拼出一个频谱完全可控的激励信号？"——把 K 根谱线逐根指定幅值 A_k 与相位 φ_k，频率全部锁定在网格 kΔf 上，叠加的结果就是伪随机信号的时域波形。
...
::: warning 工程注意
相关技术手册对系统分析（频响函数测量）的窗函数推荐为：汉宁窗用于随机激励的参考通道和响应通道；**矩形窗只用于伪随机激励的参考通道和响应通道**；力窗与指数窗用于锤击法。若测伪随机时误用汉宁窗，等于对已周期化的信号再施加一次人为调幅，幅值和相位均会失真，这是 Testlab 使用者常见的参数残留错误。
:::
...
这个公式回答的问题是："K 个正弦分量不同时撞峰，相位该怎么排？"——Schroeder 相位给出一个只依赖谱线序号 k 的确定性相位表，让各分量的瞬时频率在时间上近似均匀铺开，峰值错开、RMS 不变，波峰因子随之被压低。
...
同为 50 个分量，随机相位波峰因子为 3.06，Schroeder 相位降至 1.89：峰值从 15.3 降至 9.5，RMS 保持 5.0 不变。在相同功放电压上限下，Schroeder 版本的有效激励电压为随机相位版本的 3.06/1.89 ≈ 1.62 倍，即提高约 60%。Testlab 中的伪随机信号即采用 Schroeder 相位，可在不触发过载的前提下提高激励电平。
...
一组公开技术资料中的飞机驱动点（Driving-point）FRF 对比了猝发随机与 Schroeder 正弦伪随机，判据为相干函数。结果方向明确：猝发随机工况的相干函数在多个频段出现明显下凹；伪随机 + Schroeder 正弦的相干函数整体接近 1.0，共振峰附近也无塌陷。
...
伪随机＝按傅里叶网格"对号发车"的多正弦：频率全部落在谱线上、矩形窗零泄漏，Schroeder 相位把峰值错开让功放电压花在刀刃上——高保真 FRF 的优先激励选择。
