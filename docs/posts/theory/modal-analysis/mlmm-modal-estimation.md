...
在进入第一个公式前，先说明它回答的问题是：**用什么数学结构同时描述一整族 FRF？**其中 $N_{oi}(\omega)$ 对应物理里每个输入输出对各自的分子动态（承载该测点的振型信息），$D(\omega)$ 对应物理里全结构共享的同一组极点——就像同一栋楼（结构）的所有房间共用一根承重柱（分母），每个房间再有自己的隔断（分子）。
...
下面这个公式回答的问题是：**怎么把分式拟合变成能一次解出的线性方程？**其中 $\hat{H}_{oi}$ 对应物理里实测到的频响，$B_r$ 对应物理里公分母的系数——两边同乘分母 $D$ 之后，未知数全部以线性形式出现，代价是测量噪声从此被乘上了 $|D|$ 的尺子（这正是后文偏差的来源）。
$$
...
进入第二节，先给出核心的概率公式。它回答的问题是：**参数取某组值时，恰好测到手里这条数据的概率有多大？**其中 $\hat{H}_k$ 对应物理里第 $k$ 条谱线的实测值，$\sigma_k^2$ 对应物理里该谱线的噪声方差（信噪比的度量）——测量值离模型预测越远、且该谱线本身量得越准，这组参数的嫌疑就越大。
...
这个公式回答的问题是：**所有谱线的证据合并起来，谁是最可能的参数？**其中分子 $|\hat{H}(\omega_k) - H(\omega_k,\theta)|^2$ 对应物理里每条谱线的拟合残差，分母 $\sigma_k^2$ 对应物理里该谱线的噪声方差——方差大（信噪比差）的谱线自动被降权，这个权重不是人为规定的，是概率推导的必然结论。
$$
...
进入第三节，这个公式回答的问题是：**非线性问题怎么化为一轮一轮的线性问题？**其中 $J_a$ 对应物理里每个参数动一点、模型在各谱线上变化多少的灵敏度表，$\Delta\theta$ 对应物理里本轮的参数修正量——在当前参数点把模型当作线性，解一个加权最小二乘求增量，更新后再重复，直到代价不再下降。
