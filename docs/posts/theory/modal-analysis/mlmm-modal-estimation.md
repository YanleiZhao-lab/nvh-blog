...
先看一组能摸到问题轮廓的数字。一条 20~60 Hz、801 条谱线的实测 FRF，共振峰 30.5 Hz 附近幅值大约是反共振谷的 300 倍——同一个传感器、同一次采集、同一条曲线里，动态范围横跨 50 dB。用等权最小二乘去拟合这样一条曲线，误差并没有均匀摊在每条谱线上：幅值大的共振峰"话语权"天然大，拟合曲线会迁就它们；幅值小的反共振谷几乎被忽略，谷底的噪声反而被原样抄进模型。第四节末尾的数值实验将给出量化结果：等权拟合在共振带误差约 8%，在反共振区误差约 25%——同为"误差"，相差三倍，根源不是拟合器不努力，而是"等权"这个默认假设本身错了。另一个数字同样说明问题：对 35 Hz、12% 阻尼的重阻尼模态，Polymax 粗拟合给出的频率可以偏 2 Hz、阻尼偏四分之一，而报告需要的表述是"35.15 Hz ± 0.03 Hz（95% 置信）"——从点估计到带不确定度的区间估计，中间隔着的正是本文第二节要建立的最大似然框架。带着这两个数字（动态范围 50 dB、误差 8% 对 25%）往下读，后文的每一个公式都在回答它们背后的问题。

模态拟合的本质是为测得的频响函数（FRF, Frequency Response Function）$\hat{H}(\omega_k)$ 寻找一个有理分式模型。相关技术手册采用带标尺的矩阵分式描述，即公分母模型（Common Denominator Model）：输出 $o$ 与输入 $i$ 之间的 FRF 表示为

在进入第一个公式前，先说明它回答的问题是：<strong>用什么数学结构同时描述一整族 FRF？</strong>其中 $N_{oi}(\omega)$ 对应物理里每个输入输出对各自的分子动态（承载该测点的振型信息），$D(\omega)$ 对应物理里全结构共享的同一组极点——就像同一栋楼（结构）的所有房间共用一根承重柱（分母），每个房间再有自己的隔断（分子）。

$$
\hat{H}_{oi}(\omega_k) = \frac{N_{oi}(\omega_k)}{D(\omega_k)}
$$

分子多项式 $N_{oi}(\omega) = \sum_{r=0}^{p} A_r\, \Omega_r(\omega)$ 每对输入输出各有一个；公分母多项式 $D(\omega) = \sum_{r=0}^{p} B_r\, \Omega_r(\omega)$ 由全部 FRF 共享。其物理依据是：同一结构、同一组极点，分母理应相同；分子则承载各测点的振型信息。基函数取离散时间形式 $\Omega_r(\omega_k) = e^{-j\omega_k \Delta t\, r}$（$\Delta t$ 为采样时间间隔），即 $z$ 变换基；相关技术手册 15.4.1 节指出该形式用于优化计算速度和存储要求，同时其数值条件数好，系数不因分析频带宽窄而失衡。

可以把两种拟合路线想象成**给一群身高悬殊的人分配宿舍床位**。线性化最小二乘像按统一尺寸裁床：不管住的人是 1 米 6 还是 1 米 9，床一律 1 米 8——制作快（一次线性求解），但对高个子（共振峰，幅值大）系统性地截短，对矮个子（反共振谷，幅值小）又空得离谱；偏差不是随机误差，而是床的尺寸本身带来的系统错位。最大似然则像给每个人量体裁衣：先知道每个人的身高方差（谱线噪声方差 $\sigma_k^2$），信噪比高的谱线（共振峰，量得准）给的料多、信噪比低的谱线（反共振谷，量不准）给的料少——最终的总不合身程度（代价函数）在统计意义上最小。裁衣慢（要迭代），但版型有依据：还能顺便算出袖长误差 ±2 cm 这类不确定度（置信区间），这是统一裁床给不了的。后文的高斯-牛顿迭代与置信区间推导，都可以对着量体裁衣这幅图来读。
这个模型对系数 $A_r$、$B_r$ 是线性的，但 FRF 本身是分式。经典做法是两边乘以分母多项式实现"去分母"（线性化）：


下面这个公式回答的问题是：<strong>怎么把分式拟合变成能一次解出的线性方程？</strong>其中 $\hat{H}_{oi}$ 对应物理里实测到的频响，$B_r$ 对应物理里公分母的系数——两边同乘分母 $D$ 之后，未知数全部以线性形式出现，代价是测量噪声从此被乘上了 $|D|$ 的尺子（这正是后文偏差的来源）。
$$
\sum_{r=0}^{p} A_r\, \Omega_r(\omega_k) - \hat{H}_{oi}(\omega_k) \sum_{r=1}^{p} B_r\, \Omega_r(\omega_k) = 0
$$

其中分母首项系数归一化为 $B_0 = 1$。相关技术手册同时指出，该方程可乘以一个权函数 $W(\omega_k)$，即线性化 LS 的估计结果可以用适当的权函数修正——这正是后文重加权做法的依据。对每条谱线列一个方程，堆叠成超定线性方程组，最小二乘一次解出全部系数——求解快，但引入一个系统性偏差：**去分母之后，原始测量噪声不再等权进入方程**。设真实情况为 $\hat{H} = N/D + n$，噪声 $n$ 加在分式上；去分母后方程残差变为 $N - \hat{H}D = -nD$，噪声被乘上分母 $D$ 的模。而 $D(\omega)$ 在共振峰附近趋于最小值（极点即分母的根），在反共振谷处幅值很大——于是**共振区数据被自动降权、反共振区噪声被放大**，拟合曲线的动态范围被系统性压缩：共振峰幅值偏低，反共振谷偏深。

::: info 核心概念
- **公分母模型**：所有 FRF 共享一个分母多项式（同一组极点），分子各自独立（对应各测点振型）
- <strong>线性化最小二乘（LSFD，Least Squares Frequency Domain，即 Polymax 路线）</strong>：去分母把分式拟合化为线性 LS，参数线性、一次求解，代价是噪声模型被扭曲
- <strong>最大似然估计（MLE）</strong>：直接在分式域定义残差，按噪声方差定权，统计意义上最优，但参数非线性、需迭代求解
:::

用一个极端比例感受"权"的力量：共振峰处 $|D|$ 趋于最小、反共振谷处 $|D|$ 很大，两者在一条 FRF 里可以差出两个数量级——这意味着线性化 LS 给共振峰数据与给反共振谷数据的隐含权重差了 $10^{4}$ 倍，全由分母的取值"无意间"决定，没有一个数字是工程师亲手选的。最大似然做的事，说白了就是把这本账接过来自己管：每条谱线的权重不再是 $|D|^2$ 的副产品，而是由该谱线自己的噪声方差 $\sigma_k^2$ 决定——量得准的谱线一票顶十票，量不准的十票算一票。权从哪来、给多少、错账怎么纠正，就是第二节推导要逐一回答的问题。

相关技术手册 15.4 节还列出了 LSCE（Least Squares Complex Exponential，最小二乘复指数法）路线的结构性局限：参考（输入）自由度数大于 3 时，多参考 LSCE 估计并非总能给出可靠结果；要求频率间隔均匀；测量受严重干扰时稳定图可能不清晰；以及其中影响最大的一点——**无法提供模态参数估计的置信区间**。工程上阻尼比能否以"±10%"这类不确定度的形式写入报告，取决于该区间是否存在。
...
进入第二节，先给出核心的概率公式。它回答的问题是：<strong>参数取某组值时，恰好测到手里这条数据的概率有多大？</strong>其中 $\hat{H}_k$ 对应物理里第 $k$ 条谱线的实测值，$\sigma_k^2$ 对应物理里该谱线的噪声方差（信噪比的度量）——测量值离模型预测越远、且该谱线本身量得越准，这组参数的嫌疑就越大。
...
这个公式回答的问题是：<strong>所有谱线的证据合并起来，谁是最可能的参数？</strong>其中分子 $|\hat{H}(\omega_k) - H(\omega_k,\theta)|^2$ 对应物理里每条谱线的拟合残差，分母 $\sigma_k^2$ 对应物理里该谱线的噪声方差——方差大（信噪比差）的谱线自动被降权，这个权重不是人为规定的，是概率推导的必然结论。
$$
J_{ML}(\theta) = \sum_{k=1}^{K} \frac{|\hat{H}(\omega_k) - H(\omega_k,\theta)|^2}{\sigma_k^2}
$$

**每一步的物理意义**：方差大的谱线（信噪比差，如反共振谷、泄漏污染区）自动获得小权重；方差小的谱线（共振峰，信噪比高）权重大。这一权重分配并非人为设定，而是概率推导的直接结论——与线性化 LS 把权重固定为 $|D|^2$ 的隐含做法形成直接对照。实际操作中 $\sigma_k^2$ 由多次平均的样本方差估计（非参数噪声模型），因此求解器对噪声模型误差具有鲁棒性，也能应对动态范围很大的测量——这是相关技术手册对 ML 求解器的两点表述。
...
$J_{ML}$ 对参数是非线性的（分式模型），没有闭式解。相关技术手册采用高斯-牛顿（Gauss-Newton）算法：在当前参数点对模型做一阶泰勒展开 $H(\omega,\theta) \approx H(\omega,\theta^{(i)}) + J_a \Delta\theta$（$J_a$ 为雅可比矩阵，每行是偏导 $\partial H/\partial\theta_j$ 在该谱线的取值），代入 $J_{ML}$ 后对增量求极小——问题又回到一个加权线性最小二乘：

进入第三节，这个公式回答的问题是：<strong>非线性问题怎么化为一轮一轮的线性问题？</strong>其中 $J_a$ 对应物理里每个参数动一点、模型在各谱线上变化多少的灵敏度表，$\Delta\theta$ 对应物理里本轮的参数修正量——在当前参数点把模型当作线性，解一个加权最小二乘求增量，更新后再重复，直到代价不再下降。
...
它同时也是克拉美-罗下界（CRLB，Cramer-Rao Lower Bound）的良好近似——无偏估计器的方差不可能低于该界。ML 估计在大样本下渐近达到 CRLB，因此相关技术手册指出 ML 估计的协方差矩阵通常接近 CRLB。工程读法：只关心频率和阻尼的不确定性时，只需要分母系数对应子块的协方差，不必对全矩阵求逆——计算量小，且报告中可以给出形如"35.15 Hz ± 0.03 Hz（95% 置信）"的、有统计依据的结果。

| | <strong>线性化 LS（Polymax 粗拟合）</strong> | <strong>MLMM（ML 迭代优化）</strong> |
| --- | --- | --- |
...
- 需要阻尼/频率的置信区间支撑报告结论：ML 是相关技术手册路线中唯一能给出区间的
...
MLMM 并非以新算法取代旧算法，而是将 Polymax 的快速（线性 LS 一次求解）与 ML 的精确（分式域定权与置信区间）衔接成一条完整的分析流程：Polymax 稳定图解决"有哪些模态"，MLMM 迭代解决"参数到底多准"。工程判断标准三条——轻阻尼金属结构初拟合已足够准时不必使用；重阻尼、声腔、内饰车身、密频系统是 MLMM 的高收益场景；需要报告带置信区间的阻尼时，ML 是相关技术手册路线中唯一能给出的。初值质量决定结果上限：若稳定图上极点选择不当，增加迭代次数也无法弥补。
