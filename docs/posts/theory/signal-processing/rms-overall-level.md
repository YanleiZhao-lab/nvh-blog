...
一台电机交付前的数据复核：同一段录音、同一帧数据，时域脚本算出的 RMS 与频域谱线合成的总级差了整整 3 dB——1.414 倍。两人的原始数据完全相同，谁都没改过记录，答案却对不上账。多数时候不是谁算错了，而是频域那一路悄悄违反了"时域频域能量对账"的前提：谱线还是 Peak 格式没换算、或单位停留在 Pa² 没开根号、或加了窗没乘能量校正。本文把这条对账链拆开讲清楚：RMS 到底在量什么、谱的总级怎么合成才守恒、三项前置条件各自在补哪笔账——第四节会用一段可复现的信号实测：按对账条件合成偏差仅 0.35%，拿 Peak 格式硬算则偏大 41.9%，差的正是这 3 dB。

## 一、为什么平均值为零的信号也有能量

声压波在正负帕斯卡之间振荡，一个周期内的平均值为零。如果仅以平均值衡量信号强度，将得不到任何有效信息——平均值不是衡量信号能量（做功能力）的统计量。

想想插在市电上的电热毯。电流每秒来回反转 50 次，平均电流严格为零——可毯子分明是热的。决定发热量的是一个"等效直流档位"：把每一瞬间的电流平方（负半周翻正，发热不分方向）、取平均、再开根，把这个值接上直流电源，发热效果分毫不差。这个档位就是 RMS，"等效稳态值"的名字正由此而来。后文整条线都能用这块电热毯读下去：峰值是电流的最大摆幅，峰峰值是正负摆幅之差，RMS 是等效发热档；到了频域，每根谱线是一根独立发热丝，不同频率相互正交、各丝各热各的互不干扰，所以总级才能把各丝功率直接相加——正交性就是"互不抢热"的数学保证。

下面这个公式回答的问题是："等效发热档位"如何算成一个具体的数？其中平方对应物理里的"发热不分方向"——负半周翻正后能量才能累计；对时间积分再除以总时长，对应把全部能量摊到每一秒；最后开根号，把量纲从平方单位换回原来的单位。
...
下一个公式回答的问题是：最简单的情形——单一正弦波——等效档位是多少？答案是峰值的 0.707 倍：正弦平方在一个周期内的平均恰好减半，开根即得 0.707。这个固定的 0.707，正是后文 Peak 与 RMS 两种幅值格式之间的换算桥。
...
这个公式回答的问题是：一整条频谱折合成一个数该多大？把每根谱线想成一根独立发热丝：各丝的功率（幅值平方）直接相加、整体开根——其中每根谱线的幅值对应一根发热丝的等效档位，求和号对应"不同频率正交、各丝各热各的"这一物理前提。
