...
锤击测试里有个经典怪象：同一枚安全带张紧器，同一个人测两次，时域波形上冲击峰值附近的小波纹一次大一次小、有的台阶振有的不振，峰值读数能差出百分之十几。传感器坏了？装夹松了？都不是——把示波器接上去看，仪器自己输出的校准方波在边沿处也顶着一个小鼓包，怎么调都消不掉。更麻烦的是，这个鼓包今天大明天小，无法标定扣除。这不是操作问题，而是任何有限带宽系统对突变信号的固有反应。本文就回答三个问题：这个振铃为什么会出现、幅度由什么决定、测瞬态时该怎么设置滤波器。


## 一、时域不连续需要无限频率内容

先打个贯穿全文的比方：一支交响乐团复原一段乐谱。乐谱上的音符对应频率成分，乐团的人数对应测量带宽。演奏一段圆滑的旋律（光滑信号），几十人的室内乐团足以复原得原汁原味；可乐谱里若有一个要求瞬间从最弱跳到最强的音符（时域突变），无论再补多少乐手，只要人数有限，那一跳的两端总能听出轻微的晃动——而且有意思的是，乐手从一百人加到一千人，晃动的幅度并不见小，只是晃动持续的时间越来越短。吉布斯现象就是这个人数有限的听觉缺陷在信号处理里的化身：带宽（乐手数）管的是晃多久，滤波器形状（收声方式齐不齐）管的是晃多大。记住这条分工线，后文所有结论都能挂上来。

先看三组具体数字，再谈公式。同一枚张紧器点火脉冲，采样系统带宽开到 20 kHz，边沿后的小鼓包过冲约 18%、持续约 25 微秒；带宽压到 5 kHz，过冲仍是 18% 左右、持续时间拖长到约 100 微秒；换成缓滚降 Bessel 低通，过冲几乎归零。三个数字说了三件事：加带宽，振铃不变小、只变短；换形状，振铃才变小；而这鼓包既不是传感器噪声也不是装夹松动，是有限带宽系统对突变信号的固有找零误差，标定扣除不掉。后面的所有公式，都是在回答这三个数字背后的三个问题——方波为什么需要无限多谐波、过冲为什么收敛在 18%、振铃时长为什么反比于带宽。
...
下面这个级数回答的问题是：一个最典型的突变信号——方波——到底需要哪些频率成分才能拼出来。其中求和项里的 1/k 对应物理里第 k 次谐波的幅值权重，4/π 是把幅值归一到 ±1 的换算系数。
...
::: info 核心概念
- <strong>不连续（discontinuity）</strong>：信号幅值的突跳。它要求无限频率内容，是吉布斯现象的源头
- <strong>吉布斯现象（Gibbs phenomenon）</strong>：频率内容被截断后，时域信号在突变处出现的过冲/下冲振铃伪影
- <strong>截断（truncation）</strong>：测量带宽有限，高于带宽的谐波被丢弃
:::
...
这个公式回答的问题是：带宽只够请到前 N 位乐手时，方波会被复原成什么样子。其中 2n-1 对应物理里的奇次谐波序号（方波只含奇次谐波），f_max 是这次测量保留下来的最高频率——它正是后文振铃时长的总开关。
...
这一步回答的问题是：乐手很多时，逐个累加能否换成一鼓作气的积分。其中 u 是把谐波角频率与时间乘在一起的无量纲组合变量，Si 是正弦积分函数——它把边沿附近全部谐波干涉的结果浓缩成一个数。
...
这个公式回答的问题是：过冲到底有多大、会不会随带宽增加而消失。其中 Si(π)≈1.852 是正弦积分的最大值（对应边沿后第一个波峰），π/2 是稳态值对应的收敛值——两者之比就是那个著名的约 18% 过冲。
...
这个公式回答的问题是：振铃要晃多久。其中分母里的 2f_max 对应物理里每个振铃瓣只占截断频率的半个周期——带宽越宽、乐手越多，晃动收场越快。
...
用乐团的比方说：砖墙滤波相当于让全体乐手在同一瞬间齐刷刷收声——收得越整齐（频域越陡），场子里残留的回响（时域振铃）越明显；高斯缓滚降则像让乐手依次渐弱收场，听觉上干净得多。收声方式对应的就是滤波器形状，它与人数（带宽）无关。
