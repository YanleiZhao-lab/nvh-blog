...
> 台架上一款电机异响，时域波形平稳，RMS 与正常件接近，客户仍投诉"咔哒"声。这类问题出在信号幅值的分布形状上：冲击事件集中在直方图的尾部。峭度（Kurtosis）是量化这一特征的无量纲统计量：方波为 1、正弦为 1.5、高斯随机为 3（不减 3 口径），含冲击成分的信号显著大于 3。本文依据相关技术手册与公开技术资料，说明其定义、与峰值因子的区别，以及在声品质诊断和随机振动控制试验中的应用方法。

两款电机装在同一台减速器上做对比，声压级读数只差 0.3 dB，评审却一边倒：一款“沙沙”声可以接受，一款“咔哒”声被直接否决。更麻烦的是翻频谱也找不出差别——两条谱几乎重合，峰值表读数也在误差带内。问题既不在能量大小，也不在单一最大值，而在幅值的分布形状：被否决的那款每秒冒出几十个稀疏尖峰，绝大多数时间却相当安静。RMS 是二阶矩，对“能量何时到来”不敏感；峰值只认最大那一个，一个毛刺就能带偏。要量化“尖峰有多尖、尾部有多长”，需要一个专门的统计量。
...
打个贯穿全文的比方：把一段信号的采样点想成一座城市里所有居民的年收入，峭度就是这座城“贫富悬殊程度”的统计量。方波像人人收入完全相同的城市，没有尾部，峭度最低；高斯随机像一座常规城市，收入呈钟形分布，峭度基准记作 3（减 3 后归零）；含冲击的信号则是“平均工资没涨、却搬来了几位亿万富翁”的城市——算平均收入（RMS）几乎不动，但四次方计权相当于把最富那批人的财富放大到四次方再统计，几个极端样本就足以把总量顶上去。峭度盯住的正是尾部：偶发的大冲击像少数巨富，改变不了均值，却决定分布的形状。
...
这个公式回答的问题是：这座“城市”的平均收入与收入差距有多大——均值定中心、标准差定宽度，两者是峭度的两块垫脚石。
...
均值描述分布中心，标准差描述分布宽度，这个公式回答的问题是：分布的尾部相对高斯有多长——去均值、除以标准差完成无量纲化后，对每个点取四次方平均，再减 3 把高斯拨到零。

两者都不含高阶信息。将每个采样点标准化后取四次方平均，即得峭度（相关技术手册 式 9-14）：
...
相关技术手册对判读有明确约定：峭度为正值，表示实际分布比高斯分布有更长的尾部；负值表示尾部更短。
...
这个公式回答的问题是：用有限 N 个点估峭度能信到几分——峭度估计自身的标准差有多大，数据量要多长才够。

高次矩的代价是估计精度差。相关技术手册给出：对理想高斯分布样本，峭度估计的标准差约为

$$\sigma_{\kappa} \approx \sqrt{24 / N}$$

其中 $N$ 为采样点数。这意味着峭度估计的置信度随数据量增长缓慢：欲将 $\sigma_{\kappa}$ 压到 0.1，需要 $N \approx 2400$ 点；压到 0.05 则需约 9600 点。对非高斯分布，该标准差无法解析确定，实际精度通常更差。相关技术手册同时提示：偏斜度与峭度这类高次矩，与基于线性相加的低次矩相比鲁棒性较差，计算结果分散，使用时须谨慎——数据量不足时短帧峭度数值跳动属正常现象，判读应以足够长的记录为基础。
...
RMS 管平均、峰值管最大、峭度管尾巴：四次方计权让 3σ 的单点顶得上 81 个普通点，高斯归 0、冲击远大于 0；判读先对口径（减 3 与否），定位用滑动帧，试验用 Kurtosis Control 补 PSD 管不到的峰值分布。
