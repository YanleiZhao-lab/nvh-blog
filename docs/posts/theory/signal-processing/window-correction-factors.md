...
> 加窗在抑制泄漏（leakage）的同时，也会同时压缩信号的谱线幅值与能量。窗函数修正系数（window correction factor）通过在频域对每条谱线乘一个由窗类型决定的固定系数来补偿这种失真，但幅值校正（amplitude correction）与能量校正（energy correction）的系数并不相等——汉宁窗的幅值修正系数为 2.00，能量修正系数为 1.633，同一条谱线只能取其一。本文从汉宁窗的定义出发分步推导两种系数，给出相关技术文档中的完整系数表，说明 Simcenter Testlab 中的设置位置，以及 RMS 计算在后台自动换用能量校正值的行为。

一台风机齿轮箱在台架上做阶次切片：同一工况复测两次，4 阶分量一次读 87 dB、一次 89 dB，差了整整 2 dB。查传感器、查标定、查安装，都没有问题——最后发现两次后处理的设置里，一次是汉宁窗配幅值校正，另一次软件把同一条谱按能量校正显示。窗还是那个窗，但"乘 2.00 还是乘 1.633"这个后台选择，直接决定了谱线整体上抬多少。这不是玄学，而是一道可以精确推导的算术题：加窗把信号的幅值和能量同时压小了，而把这两个量分别补回真实值所需的系数，天生不相等。本文就把这道题从头算一遍，并说清楚什么时候该用哪个答案。
...
可以把加窗想象成给一段录音配"音量推子"：推子从零推起、中间拉满、结尾再归零。歌手站在推子最高的中间段，唱得最响的那一句（峰值）和整首歌累计的响度（能量）都被这个推子削掉了一截——峰值大约只剩一半，能量大约只剩六成。修正系数就是后期把推子拉回来的补偿增益：想让"最响一句"恢复真实电平，增益拨到一档（幅值校正 2.00）；想让"整首累计响度"对上总表，增益拨到另一档（能量校正 1.633）。一台调音台一个推子只能拨一档，一条谱线也只能乘一个系数——这就是两种校正不可兼得的直观原因。
...
这个公式回答的问题是：窗这条中间高、两端零的权重曲线，每个样本点上到底取什么值。其中 $n$ 是样本序号，$N$ 是窗长，$w(n)$ 在 0 到 1 之间——两端为 0、正中为 1，其余按余弦规律过渡。
...
这个公式回答的问题是：窗的平均权重是多少——即一条谱线的幅值会被整体压到原来的几成。把全部样本的窗值加起来除以 $N$，就是对平均压了多少的直接度量，后面简称相干增益。
...
这个公式回答的问题是：想把被压小的谱峰补回真实读数，每条谱线该乘几。答案取平均权重的倒数——压了一半，就乘回去 2。
...
这个公式回答的问题是：加窗后信号的功率（均方值）被压到原来的几成。注意这里先对窗值平方——能量类指标对应的是窗值的平方加权，而不是窗值本身，这正是两种系数分道扬镳的地方。
...
相关技术手册中「加窗后信号的能量仅为原信号能量的 61%」即按此均方根量级表述。

这个公式回答的问题是：想把被压小的能量/RMS 量级补回真实值，每条谱线该乘几。答案取功率增益的平方根——功率压到 3/8，量级就补回根号 8/3 倍。
...
物理意义：把被窗的均方权重压缩的能量/RMS 量级补回原始值。相关技术手册据此给出「加汉宁窗后的数据需倍乘 1.63 以校正能量」。

### 2.3 各窗型的修正系数表

公开技术资料中 Window Correction Factors 专题与相关技术手册的系数表给出了一致的结果：

| 窗类型 | 幅值修正系数 | 能量修正系数 | 典型用途（据相关技术手册） |
| --- | --- | --- | --- |
...
![窗函数修正系数表：各窗型的幅值与能量修正系数](/images/window-correction-factors/fig2-correction-table.png)

*（图源：网络官方公开资料）*


三点说明。其一，只有 Uniform 窗（等价于不加窗）的两种系数相同且等于 1，其余窗型两者必然不等。其二，公开技术资料中的 RMS 计算专题在能量校正语境下给 Flattop 窗的系数为 2.225，与上表的 2.26 略有出入，源于 Flattop 窗定义变体的取法不同，不影响本文的方法与结论。其三，相关技术手册的系数表还给出重复加窗的情形：汉宁窗施加两次（x2）系数为 2.67/1.91，三次（x3）为 3.20/2.11，即修正系数取决于窗类型与施加次数。
...
需要特别注意的是 RMS 计算：在图上右键 `Add Double Cursor -> X`，再 `Calculations -> RMS` 取某频段的均方根值时，**Testlab 会在后台自动把数据换算成能量校正值再计算**——即使屏幕上显示的是幅值校正的谱。按照公开技术资料对 RMS 计算前提的说明，软件在后台统一完成三项换算：谱线取线性格式（功率单位先开方）、幅值格式取 RMS、以及加窗数据施加能量校正。
...
运行结果：两个系数分别为 2.000 与 1.633，与相关技术手册的系数表一致。幅值校正把谱峰精确恢复到 1.0000，但总 RMS 为 0.8660，比真值 0.7071 偏大 22%（$2/\sqrt{8/3} \approx 1.225$）；能量校正把 RMS 恢复到 0.7071，谱峰停在 0.8165（真实峰值的 82%）。两个系数各补偿一个量，无法兼顾另一个。
...
窗把峰值和能量同时压小：补峰值乘 2.00、补能量乘 1.633，一条谱线只能选一个系数——读谱峰选幅值校正，算 RMS/能量选能量校正；Testlab 算 RMS 时后台永远悄悄换成能量校正。
