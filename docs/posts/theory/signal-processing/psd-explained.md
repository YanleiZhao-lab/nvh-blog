...
这种分歧在真实项目里代价很高。某车型路噪整改，整改前的基准谱由 A 组用 1 Hz 分辨率测得，整改后的验收谱由 B 组用 8 Hz 测得，两条曲线叠在一起，8 Hz 那条几乎全面高出 8 dB，会议室里「整改反而更差」的结论差点定案——而把两组数据各自做 RMS 求和，总量几乎没变。问题不在测量、不在整改，在于拿 Autopower 的幅值去对比本该用密度表述的随机信号。更隐蔽的是反方向的坑：同一根 200 Hz 的正弦尖峰，换一次分辨率，它在 PSD 里的数值也会跟着漂移，规范里的合格线怎么对都不齐。这两种「数字对不上」，根源是同一件事——谱线的幅值到底属于「这根线」，还是属于「这段带宽」。本文就沿着这条主线把 PSD 讲透。
...
这个过程用一个物理准确的类比来看会非常直观：把雨倒进一排量杯。同一场雨（同一份振动数据）落在桌面上，用 6000 只小杯去接，每只杯里的水位都不高；换 750 只大杯去接同一场雨，每只杯的水位自然涨上去——水位是「杯子口径」的函数，不是雨量的属性。判断雨下得大不大，看单只杯子的水位会被杯子数量骗到，把所有杯子的水量加总才是硬指标。谱分析里的 Autopower 幅值就是「每只杯子的水位」（随 Δf 变），RMS 求和就是「总水量」（不随 Δf 变），而 PSD 相当于把水位换算成「单位口径的水量」——不管用大杯小杯，换算结果都一样。这个「量雨」类比贯穿全文：随机信号像绵绵细雨摊满整张桌子（能量铺在频段上），正弦信号像对着一个点持续倒水（能量集中在一根谱线），两类信号该看哪个量，答案完全不同。


这一现象的根源在于谱线数量。下面这个公式回答的问题是：给定分析带宽和频率分辨率，频域上到底摆了多少只「杯子」？其中 $f_{\max}$ 是分析带宽，$\Delta f$ 是频率分辨率，条数由带宽除以频率分辨率给出：
...
公开技术资料里常用的“派对类比”可以说明这一关系：信号总能量是一壶饮料，谱线是杯子。随机信号的能量在频域里摊得很开，1 Hz 分辨率给了 6000 个杯子，每个杯子里只分到一点；8 Hz 分辨率只有 750 个杯子，每杯分到的自然更多。杯子数量变了，饮料总量没变。
...
PSD 的定义动作只一步。这个公式回答的问题是：怎么把「杯中水位」换算成与杯子口径无关的量？做法是把 Autopower 每条谱线的幅值（按惯例已是平方量）除以频率分辨率 Δf——相当于用每条谱线覆盖的带宽把功率摊薄成密度。
...
规范里最常被问的问题：给了我一条 PSD 曲线，怎么读出某个频段的总体振动量？这个公式回答的就是它——把 PSD 曲线在频带 $[f_1, f_2]$ 内的面积（g²/Hz × Hz = g²）开方，就是该频段的 RMS，相当于把密度乘回口径再总加：
...
运行结果要点：正弦的 Autopower 峰值三次都是 0.7071 g，保持不变；PSD 峰值则被分辨率除出 8 倍差距。这里再用一个公式把「对点倒水」的图景定量化。它回答的问题是：一根全部功率集中在单谱线上的正弦，转成 PSD 后峰值会是多少？设正弦的 RMS 为 $A$，其全部功率集中在单根谱线上，PSD 峰值为
...
尽管 PSD 峰值漂移，正弦信号 PSD 的 RMS 求和结果仍然不变——RMS 求和会自动把每条谱线的密度乘回 $\Delta f$ 再累加，补偿除以频率分辨率的操作。这也是相关技术手册建议“评估任何谱函数都先看 RMS 求和”的原因。
...
随机信号比总量用 PSD（除以 Δf 得 g²/Hz，换分辨率不变），正弦信号比单线幅值用 Autopower；拿不准就改一次 Δf——幅值跟着变的是随机，不变的是正弦。
