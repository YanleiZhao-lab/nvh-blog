...
凌晨的试验室里，工程师正对着一组数据发愁：声学通道与振动通道同时采集的同一次关门声，声音事件总比振动事件晚一拍，异响源判断的时间对不上，整晚数据面临作废。排查到最后，原因藏在意想不到的地方：录音前给声音通道随手加了一个 150 Hz 低通滤波，而振动通道没加。滤波器会引入时延，这符合常识；真正反常识的是下一步：把同一款滤波器也给振动通道加上，两个通道竟然还是对不齐。原来滤波器与滤波器并不一样：有的对高低频率一视同仁地推迟同样的时间，有的却挑着频率区别对待、时延随频率滑动。同是低通，差别为什么这么大？答案取决于数字滤波器家族里最重要的一个分叉：FIR 与 IIR。本文就沿着这条分叉往下讲。

## 一、滤波器在干什么

滤噪声、去漂移、A 计权（A-weighting）、抗混叠（anti-aliasing）——NVH 测试中滤波应用广泛，但都在做同一件事：改变时域信号的频率成分。

先用一组数字建立量级感。同样以 1000 Hz 采样、同样截止 150 Hz 的两款低通：15 系数的 FIR，无论 50 Hz 还是 400 Hz 的成分，一律推迟 7 个样本——7 毫秒，不多不少；4 阶 IIR 在 100 Hz 处推迟 3.71 个样本，到 400 Hz 只推迟 0.82 个样本，时延随频率滑动，高低频之间的相对时间关系被悄悄改写。再看计算量：FIR 每个样本要做 15 次乘加，IIR 只要 10 次。一边是“时延恒定但算得多”，一边是“算得少但时延漂移”——本文要讲清的就是这笔账怎么算、什么时候该站哪一边。

以低通为例：通带内的频率原样通过，阻带内的成分被衰减，中间是过渡带。幅度调整量既可用线性倍数表示，也可换算为分贝：

先把“衰减了多少”换成一个工程通用的读数。这个公式回答的问题是：滤波器把某个频率的幅值压到原来的几分之一，用分贝怎么表达。其中 A 比上 A0 对应物理里的输出与输入幅值之比——线性世界里“减半”这件小事，在对数刻度上变成 -6 dB，于是通带的接近 0 dB 与阻带的几十 dB 才能画进同一张图。
...
这个分叉可以用一个生活类比先画出来。把滤波器想成机场的两类行李处理线：FIR 像“直行传送带”——每件行李（每个输入样本）依次经过固定的 N 个检查站（N 个系数加权），全部过完才放行；任何一件行李的等待时间都一样长，先托运的先到，顺序与时差完全可预期，而且传送带没有回路，行李永远不会被“转丢”。IIR 像“带回旋弯道的转盘”——行李除了过固定检查站，还会被送回已经走过的弯道再检查一遍、再送回、再检查（输出反馈），少数几个检查站就能造出近乎无限的检查次数，效率高得多；但弯道一旦设计不当，行李会在回路里越转越多、堆积溢出——这就是不稳定。两类处理线的全部差异，只源自“有没有回路”这一个结构区别。下文的公式会反复回到这个画面：FIR 的输出只由输入决定，IIR 的输出会反馈回自身。

相关技术手册用龟兔赛跑作比：FIR 如乌龟——慢而稳，必定跑完全程（任何系数下都稳定）；IIR 如野兔——速度快，但有时中途倒下（可能不稳定）。两类滤波器的全部差异，源自一个数学区别。

### 从差分方程看本质区别

FIR 的输出只由输入的加权和决定：

第一台“直行传送带”开工了。这个公式回答的问题是：FIR 在第 n 个时刻的输出由哪些量决定。其中乘积项 h(k) 乘 x(n-k) 对应物理里的“第 k 个检查站给 k 拍之前的输入样本打的权重”——注意求和对象全部是输入 x，没有任何一项含输出 y，这正是 FIR 名字里“有限”的来源：脉冲输入最多撑 N 拍，之后传送带上再无行李。


$$y(n) = \sum_{k=0}^{N-1} h(k)\, x(n-k)$$

其中 $x(n)$ 为输入序列，$y(n)$ 为输出序列，$h(k)$ 为滤波器系数（相关技术手册记作 a(k) 系列），$N$ 为系数个数。当前输出只用到输入的当前值与历史值——脉冲激励注入后，响应在 $N$ 个样本后完全归零，脉冲响应有限长，这正是 FIR 名称的来历。

IIR 把输出也反馈回来：

第二台“回旋转盘”登场。这个公式回答的问题是：IIR 靠什么用更少的系数实现更陡的滚降。其中第二个求和里的 b(k) 乘 y(n-k) 对应物理里的“回旋弯道”——历史输出被重新加权送回输入端，脉冲虽已消失，它的“影子”却沿反馈回路无限传递下去，脉冲响应无限长，少数系数即可织出陡峭的频率选择特性，代价也随之埋下。
...
::: info 核心概念
- <strong>FIR（有限脉冲响应）</strong>：输出只依赖输入，脉冲响应有限长，任何系数下都稳定
- <strong>IIR（无限脉冲响应）</strong>：输出反馈回输入（递归），脉冲响应无限长，可能不稳定
- **阶数 / 抽头数**：滤波器系数个数，直接决定计算量和滚降陡度
- <strong>滚降（roll off）</strong>：通带边沿到阻带的下降速度，越陡过渡带越窄
:::
...
| 维度 | <strong>FIR（15 系数）</strong> | <strong>IIR（4 阶 Butterworth）</strong> |
| --- | --- | --- |
...
接下来三步推导解释“FIR 为什么天生线性相位”。这个公式回答的问题是：FIR 对每个频率成分的增益与相移是多少。其中指数项是复指数旋转因子——每个系数乘上各自的旋转因子再求和，就是滤波器在频域的全景响应，相位信息藏在这个复数和的角度里。
...
IIR 没有这个保证：不同频率的时延不相等，相关技术手册指出时延最大通常出现在截止频率附近（本文算例的群时延峰约 3.8 个样本，出现在 117 Hz，位于通带边沿）。通带内各成分的相对相位被改变，波形发生畸变。
...
- <strong>工作变形分析（ODS，Operational Deflection Shape）</strong>：一组振动通道中部分滤波、部分未滤，通道间相位关系被改变，动画给出的结构变形形态不正确
...
零相位滤波为什么能“时光倒流”？这个公式回答的问题是：信号先正着滤一遍、再反着滤一遍，两次滤波的总效果是什么。其中的共轭项对应物理里的“时间反转后的滤波器”——复数共轭恰好抵消掉 H(f) 的全部相位因子，只剩幅值的平方，时延被数学上严格抹平。
...
设定截止频率时必须把滤两遍的衰减叠加考虑进去。相关技术手册给出零相位滤波的完整代价清单：
