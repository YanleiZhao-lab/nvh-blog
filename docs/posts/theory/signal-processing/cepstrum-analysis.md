...
变速箱台架旁的处境正是如此：0～2000 Hz 的频谱上挤着几百根谱峰，42 齿齿轮的啮合谐波、液压泵的压力脉动、电机电磁噪声、车间背景，全部叠在一张图上。工程师想追踪啮合频率 48 Hz 的谐波族——正常件与故障件各测一段，两份频谱肉眼看去同样密、同样乱，时域波形甚至分不出哪段是坏的。差别不在谱峰的高度里，而在谱峰“排列的间距”里：齿轮成分永远等间距站队，其他设备不成队形。肉眼数不清几百根线的间距，需要一个专门把“等间距结构”从噪声背景里折叠成单根谱线的工具——这正是倒频谱诞生要解决的问题：不问每根线多高，只问哪族线每隔多少赫兹来一根。带着这个疑问往下读：下文这对齿轮的啮合频率究竟落在哪里，决定了整族谱线的队形间距。

## 一、工程问题：谱峰密集，齿轮成分难以辨认


先建立一个生活图像。菜市场里几十个摊贩同时吆喝，声浪混成一片，谁在喊什么完全分不清；但其中有个摊贩每隔固定几秒敲一下锣，你闭着眼也能跟着那节奏点头——人耳对“等间隔重复”有专门的抓取能力，而且这个抓取与锣声多响无关、与吆喝多吵无关。倒频谱就是数学版的“跟着节奏点头”：频谱上几百根谱线是几十个摊贩的吆喝，齿轮谐波族是那面按固定间隔敲的锣；取对数相当于人耳的响度感知（Weber-Fechner 定律：听感近似随声压比值的对数增长），把悬殊的谱峰高度拉平，谁也不许靠嗓门大出头；再做一次逆 FFT，相当于只问一件事——每隔多少赫兹来一根，等间距的“锣点”便折叠成一根谱线，位置就是间距的倒数。后文的每一步推导，都可以对着这幅“听节奏”的图像来读。

先把两个具体数字摆上桌：啮合频率 48 Hz 的谐波族在 0～1000 Hz 内会站出 20 排队；一根 40 rpm 输出轴的轴频只有约 0.67 Hz，它的边带族间距细到肉眼根本数不清——密谱与稀间距叠加，正是齿轮箱诊断的典型困境。

相关公开技术资料给出过一个典型案例。一对齿轮：主动轮 42 齿、转速 68.57 rpm，从动轮 72 齿、转速 40 rpm。啮合频率（Gear Mesh Frequency）按两只齿轮计算结果相同：

先给全文第一个公式。这个公式回答的问题是：一对齿轮每秒啮合多少次？其中齿数 $N_A$ 对应物理里轮齿逐个咬合的个数，转速 $n_A$ 对应物理里该齿轮所在轴每分钟的转数，除以 60 只是单位换算。算出的数字就是频谱上那族等间距谱峰的“队形间距”——后文倒频谱峰的位置将由它的倒数给出。先用直觉估一遍数量级：40 多个齿、每分钟不到 70 转，齿数乘转速再除以 60，答案必在几十赫兹量级——记住这个量级，等下在倒频谱上看到 20.8 ms 的峰时，就能立刻反应过来它正是这个数的倒数。


$$f_{mesh} = \frac{N_A \cdot n_A}{60} = \frac{42 \times 68.57}{60}\ \text{Hz} = 48\ \text{Hz} = \frac{72 \times 40}{60}\ \text{Hz}$$

其中 $N_A$ 为齿数（无量纲），$n_A$ 为该齿轮所在轴的转速（单位 r/min，除以 60 换算为 Hz）。若以轴的转频为参考改用阶次域，啮合阶次在数值上等于齿数——旋转机械相关技术手册的示例中，86 齿齿轮在 600 rpm 轴上即产生 86 阶啮合。
...
::: info 核心概念
- <strong>倒频谱（Cepstrum）</strong>：对时域信号先做 FFT 取幅值、取对数、再做逆 FFT 得到的函数，俗称"谱的谱"
- <strong>倒频率（Quefrency）</strong>：倒频谱的横轴，单位是秒但**不是时间**——相位在取对数时已经丢弃；峰的位置等于频域结构间距的倒数
- <strong>边带（Sideband）</strong>：载波频率 $F_c \pm F_m$ 两侧的峰族，旋转机械中通常由缺陷对幅值的调制引起
...
下面这个公式回答的问题是：局部缺陷如何数学地写成“幅值被周期性捏放”？其中方括号对应物理里缺陷每转一圈对啮合冲击幅度的一次“捏放”，载波 $f_c$ 对应物理里被捏放的啮合振动本身，$m$ 是捏放的深浅（缺陷程度）。它是后面所有推导的起点：边带不是凭空出现的，就是这只方括号乘出来的。
...
下面这个公式回答的问题是：为什么必须取对数、不取不行吗？其中 $S(f)$ 对应物理里齿轮发出的源谱，$H(f)$ 对应物理里结构传递路径的滤波作用，两者在频域是乘法关系——乘法叠在一起分不开；取对数后乘变加，源与路径才得以线性分开，各自成为 $\log|X|$ 上可以单独辨认的一项。
...
下面这个公式回答的问题是：“谱的谱”到底怎么算、为什么一步就够？其中内层 FFT 对应物理里把时域振动摊开成频谱，$\log$ 对应上一节的乘变加，外层 IFFT 对应物理里对频率轴上的周期起伏做一次“数间距”——频率轴上每隔 $\Delta f$ 重演一次的图案，在倒频率轴上就折叠到 $1/\Delta f$ 处的一根峰。
...
关键看两个数值：故障使 20.8 ms（$1/f_{mesh}$）峰约翻倍——调幅将边带能量叠加到每个谐波附近，梳齿加密增强；250 ms（$1/f_{shaft}$）峰增至约 2.5 倍——该峰为边带族特有，健康齿轮数据中不存在。相关技术资料的实测案例里，缺陷数据的倒频谱峰同样显著高于正常数据，倒频率 0.02083 s，取倒数正好是 48 Hz 啮合频率。
...
信噪比相差约 4 倍。这也是相关技术资料建议**选用可用范围内最粗频率分辨率**的原因：总记录长度一定时，粗分辨率意味着短帧、多帧数、更多次平均——log 域噪底方差随帧数增加而降低。若将分辨率设至 0.5 Hz、1 Hz 这一量级，帧变长、帧数骤减，噪底反而无法压低。
...
- 边带间距对应调制机理而非物理部件：旋转机械相关技术手册指出，62 齿偏心齿轮每转 2 次的调制在 60、64 阶产生边带，而系统中并不存在 60 齿或 64 齿的齿轮——判读应回到调制源（偏心、局部缺陷、不对中），而非按间距寻找部件
- 转速不稳的工况：一律转阶次域再算
:::
...
| <strong>单根离散音（电机电磁噪声）</strong> | 没有梳齿结构，无峰可看 | 窄带谱与音源识别 |
