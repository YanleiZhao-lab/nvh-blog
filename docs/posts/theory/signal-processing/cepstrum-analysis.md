...
*无调制（上）与被调制的载波（下）：调制让频谱长出边带族（图源：网络 侵删）*
...
*正弦、锯齿、方波、随机信号在时域/频域/倒频谱中的形态：只有频域含等间距结构的信号在倒频谱上有峰（图源：网络 侵删）*
...
*时域—频域—倒频谱的换算路径与各域常用单位（图源：网络 侵删）*
...
*同一对齿轮的倒频谱：缺陷（橙）在 20.8 ms 处的峰明显高于正常（蓝）（图源：网络 侵删）*

![倒频率峰读数](/images/cepstrum-analysis/quefrency-peak.png)

*q = 0.02083 s，1/q = 48 Hz，正对啮合频率（图源：网络 侵删）*
