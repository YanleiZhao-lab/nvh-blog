...
先看一个试验室里反复上演的场景：消声室里测一套扬声器总成的频响，曲线拿到手，平滑的趋势线上叠着一层细密的"梳齿"状纹波；换一只传声器、往墙边挪半米，纹波间距跟着变。有经验的工程师一眼判定：这不是产品的问题，是墙没吸干净——传声器录到的是直达声叠加回波。麻烦在后头：规范要的是自由场频响，这组带纹波的数据敢不敢直接交付？能不能把回波从数据里"抠掉"再交？常规滤波器帮不上忙——纹波周期只有约 100 Hz，陷波滤掉一片等于把产品自身的频响信息一起滤掉。本文的方法正是为这种"回声删不掉"的困局准备的。

## 一、工程问题：测量信号是"源与路径的卷积"


先把这笔账算成数字。传声器距侧墙 1.7 m，声速取 343 m/s，回波比直达声多走 3.4 m，延迟约 9.9 ms；反映到频谱上，路径项每 101 Hz（等于延迟的倒数）完成一次起伏，在 5 kHz 分析带宽内要重复近 50 次——这就是那条"梳齿"的齿距。起伏幅度由反射系数决定：a = 0.6 时，谱在 20lg(1+0.6) 即 +4.1 dB 与 20lg(1-0.6) 即 -7.9 dB 之间摆动，近 12 dB 的包络起伏足以淹没一条温和的频响趋势。锤击试验是同一机理：脉冲在台面与夹具间往返，迟到的副本叠加进主波，FRF 上同样出现周期纹波。回波既污染数据、又无法用常规滤波去除——它不是某个频点上的干扰，而是乘在每个频率上的一个"路径因子"。

试验室里敲击结构，力锤脉冲传到加速度计之前，先在台面反射一次、再在夹具反射一次——传感器收到的是一串延迟衰减的副本。声学测试更为典型：消声室吸声不完全，墙面反射使传声器录到直达声与回波的叠加。

这个公式回答的问题是：传感器收到的到底是什么。其中 $s(t)$ 是源信号、$\tau$ 是反射多走的声程折算的延迟（单位 s）、$a$ 是反射系数（无量纲）——后文全部推导都从这条"源 + 延迟衰减副本"的模型出发。
数学上写成：
...
这个公式回答的问题是：回波到底把频谱改成了什么样。其中 $X(f)$、$S(f)$ 是测量谱与源谱，括号里的 $1+a\,e^{-j2\pi f\tau}$ 就是"路径因子"——一个幅值在 $1-a$ 与 $1+a$ 之间起伏的周期项，频率周期等于 $1/\tau$，正是上文算出的约 101 Hz 齿距。
...
这里需要一个贯穿全文的类比。源与回波在频域里是**相乘**关系，好比两张透明胶片叠放在投影仪上：光依次穿过两张胶片，总透光率等于各自透光率的乘积——叠定之后，无法只去掉其中一张的图案。分光光度计的解法是换坐标：不读"透光率"，读它的负对数"吸光度"，多层材料叠放的吸光度**直接相加**，每层的贡献从此可以分开扣除。倒频谱做的正是同一件事：对频谱取对数，把"源乘路径"变成"源加路径"；回波贡献在倒频轴上聚成几枚孤立的"污渍斑"（k 倍延迟处的峰），把污渍抠掉、再指数变回"透光率"，数据就干净了。这一"抠污渍"的操作，就是 liftering。
...
这个公式回答的问题是：乘法耦合能不能拆开。对数运算把频域乘积拆成两项之和——$S$ 的贡献与路径的贡献从此分居两项、互不纠缠，这是全篇推导的转折点。
对 $X(f)$ 取复对数：
...
这个公式回答的问题是：路径项里到底装了哪些成分。幂级数把周期变化的 $\log(1+z)$ 拆成一串纯复指数 $z^k$——每一项都对应倒频轴上一个位置可预测的峰。
...
这个公式回答的问题是：回波峰长在哪里、多高。左边 $\hat{x}(k\tau)$ 是倒频轴上第 k 个峰，右边给出峰高——位置按 $\tau$ 等间距排开，幅值按 $a^k/k$ 衰减且符号交替。
...
*倒频谱拖入 Front/Back 显示：零点尖峰加镜像谱是数学产物，属正常（图源：网络 侵删）*
