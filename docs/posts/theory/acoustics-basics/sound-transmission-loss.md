...
> 发动机舱噪声经前壁板透入乘员舱、排气噪声经消声器壳体外泄——这类隔声问题均以传声损失（Sound Transmission Loss，STL）为评价指标。本文依照相关技术手册，说明 STL 的定义、计算式与频率特性，定量分析材料覆盖率与开孔泄漏的影响，并给出阻抗管法与房间法两类实验室测量路线的原理、推导与适用边界。


一辆新车做完全车声学封装，隔声验收数据却对不上账：声学材料供应商报告的样件传声损失 TL 高达 30 dB，整车前围板实测却只有 20 dB 上下——10 dB 的差距，足够让一个项目开三轮专项会。更反直觉的是接下来的排查结果：把前围板再贴一层同样的材料，改善不到 2 dB；而把线束过孔的一个胶圈补上，TL 立刻回升 5 dB。材料没变、工艺没变，隔声量却像被什么偷走了。偷走它的不是玄学，而是覆盖率与开孔泄漏这两个可定量计算的物理因素——本文第二节会算给你看：TL 30 dB 的材料只要留 1% 的泄漏孔，等效 TL 就掉到 19.6 dB。带着这个问题往下读：隔声量的账，到底应该怎么算、怎么测才作数？
...
打个比方就清楚了：屏障像一堵设在河道上的拦水坝，上游来的水（入射声能）到了坝前只有三种结局——渗进坝体被吸收（吸收，最终变成坝里的热量）、被坝面弹回上游（反射）、从坝顶或闸门漏到下游（透射）。隔声设计不是把水消灭，而是管住闸门：让尽可能少的水漏到下游。TL 就是给闸门漏多少记账的指标——下游漏过的水量与上游来水量之比，取对数写成分贝。后面会看到，覆盖率与开孔相当于坝上留了没砌好的豁口：豁口漏的水可以一比一直通下游，坝体本身砌得再厚也拦不住，这正是开篇 10 dB 差距的来源。
...
其中 $W_i$ 为入射声功率（W），$W_t$ 为透射声功率（W）。

这个公式回答的问题是：工程报表里那个 XX dB 的隔声量是怎么从能量比算出来的？其中 $\tau$ 对应物理里上一式的能量透射比，取倒数再取 10 倍常用对数，把跨越多个数量级的比值压缩成分贝标尺：
...
::: info 核心概念
- <strong>传声损失（Sound Transmission Loss，STL/TL）</strong>：入射与透射声功率之比的分贝值，衡量声学处理阻止声能透过的能力
- <strong>入射声功率 $W_i$</strong>：入射到材料表面的总声功率（W）
- <strong>透射声功率 $W_t$</strong>：透过材料到达另一侧的声功率（W）
...
<strong>覆盖率不足。</strong> 声学材料未完全覆盖板件时，裸露区域的透射声能几乎不受衰减。相关技术手册给出的对比实验：方板完全覆盖、裸露 3%、裸露 25% 三种工况相比，仅 3% 的裸露面积即可使 TL 显著下降；裸露面积从 3% 扩大到 25% 带来的进一步下降，远小于出现第一个裸露区造成的损失，且高频段影响更为显著。
...
TL 与声源无关，可用扬声器等实验室声源测量，无需在实车现场实测（in-situ）。相关技术手册将测量方法分为两大类：阻抗管法与房间法，其中房间法又有声强—声压、声压—声压两种实现：
...
这个公式回答的问题是：房间法里一侧测声压级、一侧扫声强，两个数怎么拼成 STL？其中 $L_{I_i}$、$L_{I_t}$ 对应物理里入射与透射的声强级，$S_i/S_t$ 对应两侧有效面积比——功率等于声强乘面积，所以声强级之差还要补上面积比的分贝项：
