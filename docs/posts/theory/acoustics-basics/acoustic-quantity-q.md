...
> 声学传递路径分析与声源识别文献中经常出现符号 Q，单位 m³/s²——即体积加速度（Volume Acceleration），是声学系统中与"力"对等的输入量。本文依据 公开技术资料，说明 Q 的面积分定义与推导、高频分区测量的物理约束、Q-source 互易法把 12 次敲击测量合并为 1 次激励的原理（P/F 与 A/Q 的量纲一致性），以及 Simcenter Testlab 中的通道配置要点。
