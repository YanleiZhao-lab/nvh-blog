...
> 声学传递路径分析与声源识别文献中经常出现符号 Q，单位 m³/s²——即体积加速度（Volume Acceleration），是声学系统中与"力"对等的输入量。本文依据公开技术资料，说明 Q 的面积分定义与推导、高频分区测量的物理约束、Q-source 互易法把 12 次敲击测量合并为 1 次激励的原理（P/F 与 A/Q 的量纲一致性），以及 Simcenter Testlab 中的通道配置要点。

一台电机要通过四个悬置装进车身，结构声传递路径要测 4 个悬置点 × 3 个方向 = 12 条"声压/力"传递函数。用力锤逐点敲：敲完一个方向换锤、敲完一个悬置挪麦克风，一天下来 12 组数据，还得祈祷每锤敲得一样正。而隔壁试验间同样的问题只花了不到一小时：一个尺寸像大号喷壶的声源放在驾驶员耳位，12 枚加速度计各就各位，一次激励全部并行测完。两间试验室测的是同一台车，用的输入量却完全不同——一边是力 F（牛顿），一边是一个多数人没听过的量：体积加速度 Q（m³/s²）。Q 是什么？为什么它能和"力"平起平坐？为什么它能把 12 次测量并成 1 次？本文带着这三个问题往下拆。

## 一、从 F=ma 到 Q→P：声学系统的对偶

先把 Q 和熟悉的量挂上钩。做过结构 NVH 的人都习惯了一对角色：**激励**和**响应**——锤子给力、加速度计收加速度，中间隔着一条由质量、刚度、阻尼决定的传递函数。声学系统里其实也有这对角色，只是"给"和"收"的量换了名字。理解 Q 最省力的路，就是把它当成声学世界里的"力"：结构试验里你问"加 1 N 的力，某点加速度是多少"，声学试验里你问"声源注入 1 m³/s² 的 Q，耳位声压是多少"——句式完全相同，问的量不同而已。

一个生活化的类比：把声源想成一台**吹风机对着气球**。风机每秒吹出多少空气（体积速度 U，m³/s），决定了气球充气的快慢；风机风速变化得多急（体积加速度 Q，m³/s²），决定了气球表面"被推得多狠"。房间里你感受到的"风压"（声压 P），正是这股时快时慢的气流拍在你身上的结果。注意类比里的对应关系不许错位：Q 是"体积×加速度"——面积乘上表面振动的法向加速度，就像"风量×风速变化率"，它既不是力（没有质量因子的功劳），也不是声压（那是响应不是输入），而是描述"声源往外抽送空气有多猛"的源属性。
...
::: info 核心概念
- <strong>体积加速度（Volume Acceleration，Q）</strong>：声源表面法向加速度沿面积的积分，单位 m³/s²，声学系统中与力 F 对等的输入量
- <strong>体积速度（Volume Velocity，U）</strong>：声源表面法向速度沿面积的积分，单位 m³/s；谐和振动时 $Q = \mathrm{j}\omega U$，两者相差一个频率因子
- <strong>互易性（Reciprocity）</strong>：线性系统中，A 点输入 B 点响应，等于 B 点同样输入 A 点响应——Q-source 互易法测量的物理基础
:::
...
回到吹风机类比收个尾：同一台风机（Q 相同），在浴室里脸感受到的风压和在旷野上完全不同——变化的从来不是风机，是环境。可见拿声压当"源属性"会张冠李戴，Q 才是那个不随环境搬家的量。

## 二、体积加速度 = 面积 × 加速度：从定义出发的推导

"体积加速度"这一名称初看不易直观理解，本节从体积速度的定义出发分步推导。以扬声器纸盆为例：将辐射表面离散为若干面积元 $A_i$（m²），每个面积元中心布置一枚加速度计，测得法向加速度 $a_i$（m/s²）。

<strong>第一步：面积元的体积流量。</strong> 面积元以法向速度 $v_i$（m/s）振动时，单位时间内排开空气的体积为 $A_i v_i$（m³/s），这是该面积元对声源总流量的贡献。

<strong>第二步：整个表面的体积速度。</strong> 各面积元的流量按复数求和（保留相位信息），得到声源的体积速度 $U$（m³/s）：

这个公式回答的问题是：整块振动表面一共"泵"出了多少体积流量？其中 $A_i$ 对应物理里的第 i 块面积元的大小，$v_i$ 对应该面积元的法向振动速度——复数求和意味着大小与相位一起算，不是简单的算术加：

$$U = \sum_{i} A_i \, v_i$$

<strong>第三步：对时间求导。</strong> 体积加速度是体积速度对时间的导数；面积元位置固定，求导只作用于速度，即 $a_i = \mathrm{d}v_i/\mathrm{d}t$：

这个公式回答的问题是：声源"泵气"的节奏变化有多猛？其中 $\mathrm{d}/\mathrm{d}t$ 对应物理里速度到加速度的时间导数，右端把求导下放到每个面积元——面积不动，动的只有速度：

$$Q = \frac{\mathrm{d}U}{\mathrm{d}t} = \sum_{i} A_i \, a_i$$

量纲自检：m² × m/s² = m³/s²，与 Q 的单位一致。谐和振动（圆频率 $\omega$，rad/s）时加速度是速度乘以 $\mathrm{j}\omega$，因此：

这个公式回答的问题是：同一个声源，Q 和 U 之间怎么换算？其中 $\mathrm{j}\omega$ 对应物理里谐和运动"速度超前、加速度再超前 90 度、幅值乘 ω"的关系——频率越高，同样速度对应的 Q 越大：

$$Q = \mathrm{j}\omega U$$

同一声源的 Q 与 U 只差一个频率因子，声源标定必须注明标定的是哪一个量。当表面各点同相位、同幅值振动时（低频"活塞式"辐射），求和退化为乘积，Q 等于总面积乘以加速度：

这个公式回答的问题是：低频活塞式辐射时 Q 的最快估算法？其中 $A$ 对应物理里整个辐射面积，$a$ 对应表面统一的法向加速度，没有相位抵消、面积直接当倍数：
...
因此用加速度计阵列测 Q 时，**分析的频率越高，面积元必须划分得越小**，否则大面积元内部的相位抵消会使积分结果明显偏小。相关技术手册给出的仪表板分区示例说明了这一点：要在更高频段还原准确的 Q，需将板面划分为更小的面积元，逐块布置加速度计。
...
开篇那间"一小时完工"的试验间用的正是这一招。它的底气来自互易原理，可以用**邮路**来类比：城市里从 A 邮局到 B 家的投递路线，与从 B 家到 A 邮局的路线是同一条街巷序列——路是同一套路，只是方向反过来走。声振系统同理：悬置点"用力"传到耳位"出声压"的路径，与耳位"注入 Q"传到悬置点"出加速度"的路径，是同一个传递函数的两种读法。既然路是一条，那当然选省事的走法：把费事的"12 个点轮流激励"换成省事的"1 个点激励、12 个点同时收"。
...
下面两个量纲式回答的问题是：为什么"A/Q"可以放心地当成"P/F"用？两式各自把分子分母的单位约开，若都约化为 1/m²，说明两种传递函数在量纲上同种同类——这是互易替换的第一道资格检查：

$$\frac{P}{F} = \frac{\mathrm{N/m^2}}{\mathrm{N}} = \frac{1}{\mathrm{m^2}}$$

$$\frac{A}{Q} = \frac{\mathrm{m/s^2}}{\mathrm{m^3/s^2}} = \frac{1}{\mathrm{m^2}}$$

两个传递函数都约化为 1/m²，量纲一致。这不是数值巧合——对满足互易性的线性声振系统，"悬置点加力、耳位产生声压"与"耳位注入 Q、悬置点产生加速度"两条路径的传递函数相等。相关技术手册中的对比实验确认：Q-source 互易法测得的 FRF 与力锤逐点敲击结果一致，测量时间明显缩短。
...
这个公式回答的问题是：已知声源输出多大 Q，距离 r 米处声压有多大？其中 $\rho_0$ 对应物理里空气的密度（介质"称"出压强的那杆秤），$4\pi r$ 对应点源声波向四面八方扩散后摊薄的球面积，r 越大声压摊得越薄：
