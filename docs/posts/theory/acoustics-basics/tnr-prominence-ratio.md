...
*图 1　上：纯音级低于总声级；下：纯音比紧邻临界带背景高 11 dB，可闻性由这个差值决定（图源：网络官方公开资料）*
...
*图 2　谱上可见的峰（左）与可闻的纯音（右）是两回事：高差不够，峰只是背景的一部分（图源：网络官方公开资料）*
...
*图 3　TTNR：纯音级 T 与临界带内掩蔽级 M 的差值，≥8 dB 判突出（图源：网络官方公开资料）*
...
*图 4　TTNR 公式（图源：网络官方公开资料）*

### 2.3 数字实验：六根纯音只差 3 dB 一档

公开技术资料给过一个干净的演示：2000 Hz 纯音叠加恒定随机背景，纯音按 3 dB 一档分六档增强。谱图上六根峰全都清晰可见（图 5），但 TTNR 从约 0 dB 排到 13 dB——只有超过 8 dB 的那几根才真正"出得来"，低档位的峰实际上已经熔在背景里听不出了。

![恒定随机背景上六根 2000 Hz 纯音、彼此差 3 dB：谱上六根峰全部可见，但听感上只有高差的几根能被分辨](/images/tnr-prominence-ratio/f05_six_tones.gif)
*图 5　六档纯音、谱峰全可见（图源：网络官方公开资料）*

![TTNR 计算结果：数值从约 0 到 13 dB，超过 8 dB 判 Yes；TTNR Prominent 列给出是/否结论](/images/tnr-prominence-ratio/f06_ttnr_legend.png)
*图 6　TTNR 结果表：数值档与 Yes/No 档并排（图源：网络官方公开资料）*

这个实验值得用 numpy 复算一遍，验证"3 dB 一档、8 dB 阈值"的判据行为。下面构造 2000 Hz 纯音 + 限带随机噪声，用与该演示同构的临界带扣减法估计 TTNR：
...
*图 7　TTNR 图例：数值档与 Prominent 是/否档（图源：网络官方公开资料）*
...
*图 8　PR：中间带 B 对两侧带 A、C 平均级的差（图源：网络官方公开资料）*
...
开篇那对电吹风，正是这类演示案例的原型。A 品牌两根阶次线（28、60 阶）绝对分贝高，B 品牌 159.5 阶绝对分贝低；谱彩色图上（图 11）A 的线更亮，但 PR 图（图 12）上结论反转：A 的两条音调只在跑合开头 PR 越过 9 dB（红区），随后退回背景；B 的 159.5 阶整条 rpm 区间都压在红区里——从起动到最大转速全程可闻。试听证实的是 PR 图，不是谱图。

![两品牌跑合的谱彩色图与阶次切片：A 品牌 28/60 阶分贝更高，按绝对分贝应判 A 更吵(/images/tnr-prominence-ratio/f12_spectral_orders.png)
*图 11　谱图视角：绝对分贝上 A 品牌更高（图源：网络官方公开资料）*

![PR 图结论反转：A 品牌只在跑合开头突出（红区短），B 品牌 159.5 阶全程压在 9 dB 阈值之上](/images/tnr-prominence-ratio/f13_pr_map.png)
*图 12　PR 图谱视角：B 品牌的音调整程突出（图源：网络官方公开资料）*
...
*图 13　谱图与 PR 图对照：B 品牌的贯穿型音调只在 PR 图上暴露（图源：网络官方公开资料）*
...
*图 14　部件单独测试会高估音调：缺少应用场景的背景掩蔽（图源：网络官方公开资料）*
...
*图 16　光标计算路径：加 Single X Cursor 后右键选 Calculations（图源：网络官方公开资料）*
