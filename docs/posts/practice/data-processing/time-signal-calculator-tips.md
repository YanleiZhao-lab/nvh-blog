...
先看一个真实得不能再真实的场面：整车转毂试验收工第三天，分析同事打来电话——两个轮心加速度通道忘了设置低通，波形毛得没法看；频谱里还冒出一整族 8 Hz 的竖条纹，从低频一路排到几百赫兹，接地环路把整批数据都污染了。转毂重新排期要等三周，样车后天就得还给别的项目组。这种"数据测完才发现少做了一步"的窘境，几乎每个测试工程师都撞过——而大多数时候正确答案不是重测，是把补救搬回数据端：相减、滤波、积分、造一个虚拟转速通道，在一张公式表里逐行搭出整条处理链。
...
给全文备一个贯穿的类比：这张公式表就是一间**中央厨房**。原始数据是冷库里只进不出的食材——任何工序都不许直接动它，每道工序的产出一律 Save As 装盘另存；每个公式行是一道工序，洗菜（滤波）、切配（通道相减）、腌制（积分）、摆盘（生成虚拟转速），上一道工序的产出直接进下一道；整间厨房的菜谱（保存成模板的公式表）写一遍就能反复用。这个类比里有一条物理上严格的对应：刀工再好，也切不出变质部分被污染前的味道——坏数据段（过载、断线、毛刺）必须先挑出去再加工，数学运算只会把腐味拌得更均匀。后文每个函数，都可以回到这间厨房找位置。
...
这个公式回答的问题是：传感器千分之一的零偏，凭什么几秒后能把位移曲线顶出屏幕——积分是逐秒记账的累积过程，常量成分不管多小，每一秒都会足额存进账户，时间一长小钱也滚成主角。

$$v(t) = v_0 + \int_0^t a(\tau)\,\mathrm{d}\tau$$

若加速度信号含直流偏置 $\bar{a}$（传感器零偏或调理电路漂移），这部分常量单独积分，得到随时间线性增长的速度漂移：

这个公式回答的问题是：漂移以多快的速度长大——零偏 $\bar{a}$ 是常量，积分一次后速度漂移随时间线性增长，每秒的增量恰好等于零偏本身，一滴不差。

$$\Delta v(t) = \int_0^t \bar{a}\,\mathrm{d}\tau = \bar{a}t$$

速度漂移再积分到位移，漂移项变为时间的二次函数：

这个公式回答的问题是：为什么偏偏是位移最遭殃——零偏被积分两次，漂移按时间平方增长，记录时长拉长 10 倍、漂移就放大 100 倍，这正是直接积分"越积越飘"的定量根源。
...
这个公式回答的问题是：谐波滤除为什么必须先有 RPM 通道——函数按"频率除以基准转频"的阶次扣谐波，8 Hz 定频干扰配上 8 Hz 方波基准，全部倍频恰好落在 1、2、3……整数阶上，一把对齐、整族扣净。

$$\text{order} = \frac{f}{f_{\mathrm{ref}}}$$

8 Hz 固定频率谐波配上 8 Hz 方波生成的 RPM 基准，各倍频正好落在 1、2、3……整数阶上——族内所有谐波一次扣除。函数的主要参数与相关技术手册对齐：samples_per_rev 决定角域每转采样点数，可滤除的最大阶次为其一半；nr_of_cycles_for_avg 决定计算平均谐波所用的循环数，阶次分辨率等于其倒数（如 10 个循环对应 0.1 阶分辨率）。
...
这个公式回答的问题是：量"数据散布多大"的尺子怎么造才不会被毛刺污染——中值只数排名、不看幅值，离群尖峰既搬不动中值、也几乎抬不高 MAD，尺子本身就是抗污染的。
...
这个公式回答的问题是：坏点判定的门槛定在哪——1.4826 把 MAD 换算到与正态标准差同一量纲的尺度上，$k = 3.5$ 即"偏离局部中值约 3.5 倍等效标准差"的点判为坏点。
...
公式表是数据的后厨：原始数据冷库只读，工序逐行串联——先挑坏段再加工、先滤波再积分、毛刺交给 MAD、谐波配上虚拟转速；能在数据端救回的数据，绝不动测试资源重测。
