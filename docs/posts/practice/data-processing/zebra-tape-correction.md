...
实测前一天下午标定还一切正常：激光探头对着贴好斑马带的曲轴，转速通道干干净净，2 阶扭振清清楚楚。第二天正式采数据，colormap 低阶段却多出一条笔直的阶次线，1 阶附近能量异常大，扭振角位移积分出来一地锯齿。检查探头、擦镜片、重贴带子，问题依旧。真正的原因往往不在仪器，而在带子自己身上：缠带收尾处的接缝每转经过激光一次，几何上只错了一格，经过"间隔取倒数算转速"的运算放大之后，一个 ±8 rpm 的真实扭振信号旁边站着一位 −415 rpm 的每转一次假跳变。这条伪影恰好落在扭振分析最关心的低阶区间，平均消不掉、滤波分不开——本文就讲它从哪来、怎么认、怎么修。

## 一、接缝：缠绕收尾引入的系统性误差

先看测量原理。激光垂直照射带面，第 $i$ 条白纹经过时记一个脉冲，相邻脉冲的时间间隔 $\Delta t_i$ 对应一段固定的圆周角度。设带子标称每转 $N$ 条纹——即每转脉冲数（Pulses Per Revolution, PPR）——则每个脉冲间隔对应的角度增量为——往下第一个公式就回答这个问题。

往下第一个公式回答的问题是：条纹把一转切成多少格、每格占多少角度——其中 $N$ 对应物理里的每转条纹数（PPR），$360^\circ$ 是圆周总角度，两者相除就是每个脉冲"负责"的角度格宽度。

$$\Delta\theta = \frac{360^\circ}{N}$$

这是条纹提供的角向分辨率。由角度与时间的关系，该角度区间内的平均转速为多少？

这个公式回答的问题是：从一段脉冲间隔反算这一格里的平均转速是多少——其中 $\Delta t_i$ 对应物理里的第 $i$ 个脉冲到达时刻与上一个脉冲的时间差，$N\Delta t_i$ 是转完一整圈所需时间的估计，60 把"每秒转数"换算成工程上习惯的 rpm。
...
打个生活中的比方：接缝之于斑马带，就像卷尺末端铆接处那一小段刻度误差之于整把尺子。尺子其余刻度均匀，量桌子长度不受影响；但如果你一圈圈连续走尺（相当于轴一圈圈转过激光），每次经过铆接处都会多数或少数一个固定量，走一百圈误差就固定出现一百次——它不是随机读数抖动，而是每次都踩在同一个位置的系统性偏差。把每次经过都平均一下？没用，因为它每次都朝同一个方向错。斑马带的接缝同理：错的是圆周上一个固定角度位置的条纹间距，每转一次、方向一致、形状可重复。这也预示了修正思路——不能靠统计平均，只能回到几何本身，把脉冲按理论角度重新摆正。
...
误差幅值还会被"间隔倒数"运算显著放大。设豁口使第 $j$ 个脉冲间隔从与转速一致的理论值 $\Delta t_j$ 拉长为 $(1+\varepsilon)\Delta t_j$（$\varepsilon>0$ 为相对加长量），其余条纹间隔不变。由转速公式，激光系统在该处反算出的转速变成了多少——且看这一步如何把几何缺陷放大成数据里的主导误差。

这个公式回答的问题是：豁口让间隔拉长后，激光系统反算出的转速变成了多少——其中 $(1+\varepsilon)$ 对应物理里的间隔相对拉长倍数，它直接进了分母，所以转速按同样的比例缩水。

$$\hat{n}_j = \frac{60}{N\,(1+\varepsilon)\,\Delta t_j} = \frac{n_j}{1+\varepsilon}$$

对应的转速误差为多大？

这个公式回答的问题是：这一个脉冲间隔上的转速误差到底有多大——其中 $-\varepsilon/(1+\varepsilon)$ 是相对误差系数，乘以该处真实转速 $n_j$ 得到绝对误差；负号说明豁口（$\varepsilon>0$）永远把转速往低里报。
...
这个公式回答的问题是：修正时第 $k$ 个脉冲应该放在哪个角度上——其中 $k$ 对应物理里的脉冲序号（接缝也占一个序号），$360^\circ/N$ 是理论角度格宽度，取模把角度折回 $0\sim360^\circ$；重排只认"第 $k$ 个脉冲对应第 $k$ 个格"的标称几何，不看实测间隔。
