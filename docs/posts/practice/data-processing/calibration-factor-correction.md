...
先看一个真实的翻车现场：某整车转毂试验做完三天，NVH 组正在写报告，有人顺手把一组悬置加速度的峰值与上一轮对标数据一比——差了整整 10 倍。是传感器坏了？粘贴脱落了？还是采集卡出问题了？逐一排查全部正常。最后翻出试验当天的装机照片才发现：库里领加速度计时拿错了型号，装的是 10 mV/g 的 A 型，通道设置里却按 100 mV/g 录入——软件老老实实地把每 10 mV 当成了 0.1 g。三天台架工时、整车租用费用都花出去了，数据还能要么？答案是：能，而且只需要乘一个比例因子。本文要讲的就是这个因子的来历、用法，以及哪些错误属于“乘任何系数都救不回来”的边界。
...
灵敏度录入错误时，换算结果按同一比例整体偏离。公开技术资料 中的典型案例：本意使用 100 mV/g 的 B 型加速度计，实际安装的是 10 mV/g 的 A 型，通道设置中却按 100 mV/g 填写。
...
这个修正的逻辑，用**汇率换算**来类比再合适不过。采集卡记下的电压好比一笔外币存款，数额从头到尾没变；灵敏度就是汇率——软件按录入的汇率把外币换算成本币，汇率录错了，账面本币数额就整体偏多或偏少，但外币存款本身分毫未损。修正不需要回到银行重存一遍，只要按正确的汇率重算一遍账面：金额乘以“错汇率除以对汇率”即可。整个流程里有两个不变式：外币数额（电压）永远不变，金额的相对结构（哪笔多哪笔少、频谱上哪个峰高哪个峰低）永远不变——变的只是刻度。而下面要讲的边界也同样直白：如果外币存款本身被撕掉了一角（过载削波、量化损失），再精确的汇率也补不回那一角。
...
这个公式回答的问题是：**软件当时到底按什么系数把电压折算成了工程单位？**它就是“毫伏除以灵敏度”这一步除法的数学记法——录入的灵敏度是多少，每毫伏就被折成多少 g，所有数据都带着这个系数的烙印。
...
这个公式回答的问题是：**如果当初录对了灵敏度，正确的读数应该是多少？**同一个电压、同一个除法，只是分母换成真实灵敏度——把它与上一式并排放着，修正因子的雏形已经呼之欲出。

$$EU_{act} = \frac{V}{S_{act}}$$

这个公式回答的问题是：**已存盘的错误数据乘几，就能变成正确数据？**两式相除把恒定不变的电压 $V$ 消掉，只剩两个灵敏度之比——一个无量纲因子。它是整篇文章的核心：不需要碰原始电压，一步乘法完成修正。
...
这个公式回答的问题是：**修正量折合多少分贝？**幅值比先平方再取对数（系数 20 的由来），把比例因子换算成工程师口头的 dB 量——拿错 10 倍即 ±20 dB，顺手就能心算核对。
...
动手修正前应先确认"错在哪里"。相关技术文档给出了两个核查入口：
...
电压是外币、灵敏度是汇率：汇率录错只须按“错汇率除以对汇率”重算一遍账面（拿错 10 倍即 ±20 dB），外币本身分毫未损；但存款被撕了角（过载、量化损失），任何汇率都补不回来。
