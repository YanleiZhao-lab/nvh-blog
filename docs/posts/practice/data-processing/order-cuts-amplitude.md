...
同一组变速箱升速数据，两位工程师各切了一条 4 阶：一位的曲线峰值 78 dB，稳稳压在 82 dB 的目标线以下，出具"合格"结论；另一位峰值 85 dB，超限 3 dB，要求设计整改。数据是同一份，阶次也是同一个 4 阶，差出来的 7 dB 到底是谁的手艺问题？复查发现两人带宽一个填了 0.05 阶、一个填了 0.60 阶——过窄的带宽把涂抹开的阶次能量切丢了一半，过宽的又把旁边 4.3 阶的强成分算了进来。带宽这一格参数，比传感器标定更能左右结论。带着"这一刀应该切多宽"的问题，本文把阶次切的幅值账算清楚。

## 一、阶次切的幅值是怎么算出来的

阶次切不是"读出某一根谱线"，而是**能量求和**。对每个转速增量（RPM increment），在指定阶次周围取一个频带，把带内的能量按均方根（Root Mean Square, RMS）加起来，作为该 RPM 点的幅值。相关技术手册对阶次切的定义即"沿某阶次切取，得到该阶次的 RPM—幅值曲线"。

阶次切很像切生日蛋糕分能量。整张蛋糕是这一转速增量下的频谱，每块蛋糕上的一朵奶油花对应一根谱线；想把"属于 4 阶的那一份"端给客人，不能只铲走一朵花——花周围的奶油（窗函数涂抹开的能量）也是这一份的，得整块端走。铲子太窄，花瓣铲碎了、奶油留在盘子里（带宽过窄漏能量）；铲子太宽，把邻座 4.3 阶那块的角也切了进来（带宽过宽吃邻阶）。而且不同转速下的蛋糕大小还在变：转速越高阶次线对应的频率越高、涂抹越宽，铲子的宽度要不要跟着变，正是后文四种带宽模式要回答的问题。
...
先给这条公式一句导读：它回答的问题是"这一刀切下来的总幅值是多少"，其中 $A_k$ 对应物理里第 $k$ 根谱线的幅值、求和上下限 $k_1$、$k_2$ 对应落在带宽内的谱线范围、开方前的平方和对应带内总能量。有了这层对应，再看三步推导：
...
相关技术手册指出：结果可以相差许多 dB，**是否超过临界目标可能就取决于阶次是怎么处理的**。所以每切一条阶次都应回到 colormap 核对，目视确认带宽包住了目标阶次的能量、又没有覆盖邻近阶次。
...
下面两条换算公式各给一句导读。$f_{\text{rot}} = n/60$ 回答的问题是"转速表读数对应每秒转多少圈"，其中 $n$ 对应物理里以 rpm 计的转速、60 对应秒与分的换算系数；$f = O\,n/60$ 回答的问题是"每转发生 $O$ 次的事件，其频率是多少"，其中 $O$ 对应每转事件次数（阶次的定义本身）。

换算基础是阶次—频率关系，从定义分两步推导。第一步，转速换转频：转速 $n$（单位 rpm）是每分钟转数，除以 60 得转频 $f_{\text{rot}}$（单位 Hz）：

$$f_{\text{rot}} = \frac{n}{60}$$

第二步，乘上每转事件次数：阶次 $O$ 的定义是每转事件次数（相关技术手册原文"Order – Number of events per revolution"），故事件频率为：

$$f = O \cdot f_{\text{rot}} = \frac{O\,n}{60}$$

相关技术手册算例：600 rpm 的 2 阶振动，$600/60 = 10$ Hz，乘 2 得 20 Hz，与式一致。这条关系同时决定了阶次线是 colormap 上过原点的斜线。

### Order 模式

这条公式回答的问题是"固定的阶次宽度 $\Delta O$ 在每个转速上对应多宽的频率窗口"，其中 $\Delta O$ 对应物理里的阶次带宽（铲子宽度，以阶为单位）、$n$ 对应当前转速——两者相乘除以 60，就是铲子在频率轴上扫过的赫兹数：
...
这条公式回答的问题是"按阶次值的百分比取带宽时，频率窗口随转速和阶次各怎么变"，其中 $p$ 对应百分比（如 10% 记 0.10）、$O$ 对应被切阶次本身——百分比乘阶次得到阶次带宽 $\Delta O$，再套用 Order 模式的换算：
...
这条反解公式回答的问题是"固定的频率带宽 $\Delta f$ 折算到阶次轴上有多宽"，其中 $\Delta f$ 对应物理里以 Hz 计的恒定带宽、$n$ 对应当前转速——转速越高，同样的赫兹数折算成的阶次宽度越窄，铲子在阶次轴上随转速收口：
...
- 源文示例为 2500 Hz 偏移的阶次线，机械阶次不会这样，PWM 类才会（相关技术手册给出 PWM 基频常见值 2500/5000/10000 Hz）
:::
