...
> 手里只有加速度信号，交的报告却要位移——悬架行程、部件间隙、动画变形都绕不开。直接 DOUBLEINTEGRATE 一下，得到的位移动辄几米，明显失真。这篇从积分的数学定义拆解漂移的来源，给出相关技术资料 FAQ 630 的五步处理流程和频域积分替代路线，每一步跳过会出什么后果都有实测图对照。

冬夜的试验车里，一位工程师盯着屏幕上缓缓爬升的曲线陷入自我怀疑：同一台车、同一段路面，昨天算出的悬架位移还是 12 mm，今天同样的数据重新积分一遍，位移竟然爬到了 1.2 米——比整车的离地间隙还大出一个数量级。传感器没换、通道没动、公式没改，唯一的区别是今天这段记录比昨天长了 20 秒。问题不在测量，而在积分这个动作本身：加速度信号里千分之一 m/s² 量级的直流偏置，在双重积分的数学结构里会以时间平方的速度膨胀，记录每延长一秒，虚假位移都在加速生长。更麻烦的是，这类漂移在时域波形上常常“看起来很合理”——一条光滑的低频摆动，很容易被误读成车身的真实俯仰。读懂本文的两条推导与五步流程，就能解释“为什么位移超物理行程”，更重要的是在下一次看到米级位移时，知道先查哪一步。

## 一、工程场景：为什么总在积分

最可靠的位移当然来自直接测量——位移传感器、激光测振仪。但实际项目里经常来不及：车上布了二十个加速度计跑耐久，回来才发现客户要悬置位移；台架测的是加速度，间隙校核却要位移。积分（integration）往往成了唯一的补救手段。相关技术手册层面这也是常规操作：相关技术手册第 17 章明确指出，位移、速度、加速度的工作振型（ODS）之间可以通过一次或二次积分/微分互相转换（式 17-7～17-9），依据的正是同一套关系。
...
打个贯穿全文的比方：积分像是**往一个没有刻度的水缸里注水，再问缸里现在有多少水**。加速度是水龙头的流速，积分一次得到水量（速度），积分两次得到水位（位移）。麻烦在于两件事：第一，水龙头哪怕只滴漏——一个肉眼几乎察觉不到的恒定小偏置——一小时下来累积的水量完全可以淹没你真正想量的那一次“泼水”（真实振动）激起的几厘米波纹；第二，滴漏的累积是单向的、不回头的，时间越长水位越高，而真实振动的波纹却只在原地涨落。直流偏置、传感器温漂就是那个滴漏，真实振动就是那一瓢水。后文的去趋势（DETREND_AC）相当于先把缸擦干、堵住看得见的滴漏；高通滤波（FILTER_HP）相当于在缸底开一个恒定的小排水口，让慢漏的水边进边排，只留下真正的“泼水”事件。这个“注水-滴漏-排水”的画面会贯穿全文：每一步处理，都是在管住某一种“不该进缸的水”。
...
这个公式回答的问题是：只知道每一刻的加速度，位移从哪里来？它把位移拆成“加速度历程的双重积分”与“初始条件 $v_0 t + x_0$”两部分——后者正是漂移的第一来源：哪怕加速度完全为零，一个非零的初速度 $v_0$ 也会随时间线性长出位移。
...
这个公式回答的问题是：一个恒定的假加速度，积分两次后到底“假”多少？答案随 $t^2$ 增长——记录时间翻倍，虚假位移翻四倍。用水缸的话说：滴漏速率恒定，水位却越涨越快。
...
这个公式回答的问题是：同样幅值的加速度，为什么低频的位移贡献大得多？分母是角频率的平方——频率每降为十分之一，位移放大约一百倍。它同时揭示了治漂移的代价：高通滤波砍低频，砍掉的既是噪声也是真实的低频位移。

幅值除以 $\omega$，相位滞后 90°；第二次积分得 $x(t) = -\frac{A}{\omega^2}\sin(\omega t)$，幅值再除以 $\omega$，相位累计反转 180°。位移幅值：

$$|x| = \frac{A}{\omega^2} = \frac{A}{(2\pi f)^2}$$

也可以从频域看：积分算子在频域是乘 $1/(j\omega)$，双重积分就是乘 $1/(j\omega)^2$。当 $f \to 0$，这个增益趋于无穷——这正是相关技术资料在社区问答里指出的："频率域积分是 1/ωj，频率接近 0 时积分就是无穷大"。传感器的低频噪声、1/f 噪声、残余漂移全被平方放大。
...
这个公式回答的问题是：整个处理链条最终长什么样？五步顺序不可交换——先去趋势、再升采样、然后积分、再降采样、最后高通——每一步都在为后一步铺路，顺序一乱前面堵住的误差又从后面漏回来。
...
- **截止频率取多少**：相关技术文档给出的日常取值 1～2.5 Hz；不关心 5 Hz 以下运动的话取 5 Hz 也可。下限受传感器可用频带约束（ICP 加速度计低频本来就不准），上限受所关心的最低结构模态约束
...
图谱域还有一个顺手的技巧：扭振（torsional vibration）分析里转速波动阶次想换成角度位移显示，右键 Y 轴 → Processing → Integrate (Single)，一步完成（与旋转机械相关技术手册 TIP 2 的操作一致）；峰值/RMS/峰峰值（peak-to-peak）在同一菜单的 Section Scaling 里切换。
...
双重积分把直流偏置吹成 $t^2$、把 1 Hz 噪声放大一万倍：积分前 DETREND_AC 去趋势、Simpson 前后升降采样、积分后 1～2.5 Hz 高通清常数；只要级值就走频域，清零首两根谱线或 1 Hz 起算。
