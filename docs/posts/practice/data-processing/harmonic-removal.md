...
复测报告打回来那天，工程师盯着两张 colormap 看了很久：同一台样机、同一工况，上一轮的数据干净利落，这一轮却多了好几条竖条带，50 Hz、100 Hz、200 Hz 整齐排开，把 200~800 Hz 的共振包盖得严严实实。查接地、查屏蔽、查传感器，全都没问题——干扰来自厂房里新装的一台变频器，经电源线窜进了采集系统。重测要排三天台架，而数据本身是好的，只是混进了一族"知道频率、就是赶不走"的谐波。还有更隐蔽的：扭振数据里那个可疑的 1 阶，最后发现是 zebra 盘装偏了 0.05 mm，每转一圈条纹先密后疏，制造出一个假扭矩波动。这两类问题有一个共同解法：不滤波，而是把干扰"估计准、减干净"。本文讲的 Harmonic Removal 就是这条路线。
...
在进入算法之前，先给谐波去除一个贯穿全文的生活类比：**它就像合唱团里"抠掉伴奏、只留人声"的反向操作——先录一段纯净伴奏，再从混音里逐字减掉**。想象一段现场录音：歌手（你要的信号）和伴奏（干扰谐波）叠在同一条音轨上，频谱上缠成一团，均衡器怎么拉都伤人声。聪明的办法是让伴奏单独再奏一遍录下来（角域循环平均得到"平均循环"），然后把这段伴奏从混音里逐点对齐减掉（逐循环相减），剩下的就是干净的人声。关键在"对齐"二字：伴奏每遍都从同一拍起（整数阶与转角严格同步），所以能一遍遍叠加取平均而不消失；台下观众偶尔的咳嗽每遍位置不同（非同步成分循环间相位漂移），平均几次就抹平了。后文讲四步算法、阶次分辨率、参数权衡时，都可以回到"录伴奏、减伴奏"这四个字来对照。
...
公开技术资料里的经典例子：信号含 1 阶和 2.3 阶两个成分。平均循环里只剩 1 阶（以及 1 阶的整数倍，如果存在），2.3 阶因不同步被平均掉；原信号减去平均循环，剩下的恰好是干净的 2.3 阶。一减一得，两个成分彻底分离。
...
这个公式回答的问题是：**角域里怎么给任意一个成分记账**。它把信号拆成一枚枚阶次硬币——转角每走一圈，第 $k$ 阶硬币转了 $k$ 圈；$k$ 是不是整数，就是后面同步/不同步分野的全部来源。
...
这个公式回答的问题是：**相邻两个循环之间，一个成分错开了多少相位**。整数阶错开量恰好是整圈（模 $2\pi$ 为零），所以每遍波形严丝合缝地重叠——这就是录伴奏时伴奏不消失的原因。
...
这个公式回答的问题是：**非整数阶每循环漂移的角度由什么决定**——只看阶次的小数部分。2.5 阶每遍错半圈（180 度），两遍就互相抵消；2.02 阶每遍只错 7.2 度，得平均很多遍才抹得平。
...
这个公式回答的问题是：**平均 $N$ 个循环后，不同步成分还剩多少幅值**。它是等相位差矢量求和的封闭解（Dirichlet 核），分母的 $N$ 说明幅值大体按循环数倒数收缩——40 个循环平均，漂移成分约剩四十分之一。
...
这个公式回答的问题是：**平均循环数这个参数到底买到什么**——买到阶次分辨率 $1/N$。它同时划出方法的边界：与目标信号频差不足 $1/N$ 阶的干扰，循环平均分不开，别指望参数救场。

$$
\Delta k_{\min} \approx \frac{1}{N} \quad (\text{阶次分辨率} = 1/\text{平均循环数})
$$

这一步的物理意义：与相关技术资料中"10 个循环平均对应 0.1 阶分辨率"的说明一致——两个阶次差小于 $1/N$ 时，二者在 $N$ 循环平均下的衰减比都接近 1，无法区分。
...
谐波去除是先录伴奏、再减伴奏：整数阶每循环严格对齐，循环平均原样留下；非整数阶循环间漂移，平均后按 1/N 收缩——一加一减，频域分不开的成分在角域各回各家。
