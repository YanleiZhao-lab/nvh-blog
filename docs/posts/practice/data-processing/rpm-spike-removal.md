...
先建立一个贯穿全文的类比：**转速测量就是体育场上掐秒表的裁判**。裁判并不“知道”运动员跑多快，只在运动员每次踏过计时线时按一次表——相邻两次按表的时间差就是圈速；转速通道同理，激光照在条纹面上，每扫过一条条纹记一个到达时刻，用时间差换算转速。这个类比能把三类毛刺全部装进去：看台上横幅飘过挡了一下视线，裁判那一次表按偏了零点几秒，圈速曲线上立刻多出一个假峰或假谷——这是偶发毛刺；计时线本身画歪了几厘米，每一圈都在同一位置错同样的量——这是条纹制造偏差；收尾处胶带贴豁了口，相当于计时线旁多出一段空档——这就是接缝误差。后文每个公式，都可以放回“裁判按表”的场景里理解。

这个公式回答的问题是：裁判两次按表之间隔了多少秒，怎么换算成每分钟多少转？其中 60 把“每秒转数”换算成 r/min，$P$（PPR）对应每转按表的次数——每转按 $P$ 次表，一个间隔就只代表 $1/P$ 转，$\Delta t_k$ 就是相邻两次按表的时间差。
...
这个公式回答的问题是：裁判按表的手抖了一下，转速会错多少？答案是等比例反号传递——计时的相对误差原样变成转速的相对误差，负号表示计时偏长算出的转速偏低、计时偏短则转速虚高。
...
下面这个公式回答的问题是：手里没有“真值转速”可对照时，怎么判断窗口里哪个脉冲是毛刺？答案是让大多数脉冲自己当裁判——中位数对极端值免疫，先取窗口内转速的中位数作为“正常水平”，再统计所有脉冲偏离这个水平的典型幅度，MAD 就是这把“典型偏差”的标尺。其中里层的 median 对应物理里的位置基准，每个脉冲与中位数的绝对偏差对应它的偏离量，外层再取一次中位数，得到不被毛刺自身带偏的散布度量。
...
这个公式回答的问题是：想看清第几阶的扭振，每转至少要“按几次表”？它是采样定理在角度域的翻版——每转 $P$ 个脉冲相当于角度域的采样点，要分辨第 $O_{\max}$ 阶（每转振荡 $O_{\max}$ 次）的波动，每个周期至少需要两个采样点。
