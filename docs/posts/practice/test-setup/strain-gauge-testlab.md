...
所以 公开技术资料专门用一整篇文章讲“在 Simcenter Testlab 里测应变片”：硬件（VB8 卡）、接线（桥型与 LEMO 引脚）、软件（两个界面的通道设置）、标定（Bridge Nulling 与 Shunt Calibration）、归零（Zeroing/Balancing），直到测量（含自动分流序列）。本文化读官方全文，配公开资料插图与两张设置对照表，经典 Testlab 与 Testlab Neo 两条路各走一遍。
